"""
VLearn Form 4 Geography — Automated Test Suite for Topic 1: Land Reclamation and Rehabilitation

Verifies:
  1. Target Subject, Grade, Curriculum mapping (Geography, Form 4, 844)
  2. Topic 1 existence and ordering
  3. Exact 5 Learning Units and 5 Lessons
  4. Exact 74 Pages across the 5 Lessons
  5. 100% presence of sanitized SVGs on all diagram blocks
  6. 100% presence of verified Wikimedia URLs and attributions on all photographic blocks
  7. Zero bracket citations ([1], [121], [S1, p. 1]) across all blocks
  8. KaTeX / LaTeX formatting integrity (no unescaped implementation syntax leaks)

Usage:
  ./venv/bin/python curriculum/test_form4_geography_topic1.py
"""
...
def test_form4_geography():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 GEOGRAPHY TOPIC 1")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="Land Reclamation and Rehabilitation").first()
    assert topic, "Topic 'Land Reclamation and Rehabilitation' not found!"
    ...
    expected_pages = {
        ...
        3: 14, # Swamp Drainage & Pest Control
        4: 18, # Rehabilitation & Kenyan Schemes
        5: 16  # Netherlands & Comparative Analysis
    }
    ...
    assert total_pages == 74, f"Total pages is {total_pages}, expected 74"
    print(f"[PASS] 5. Total Pages across all 5 Lessons: {total_pages} (Matches target: 74)")

    # 6. Check Visual Diagram SVGs
    diagram_blocks = [b for b in all_blocks if b.block_type == "suggested_diagram"]
    assert len(diagram_blocks) >= 14, f"Expected at least 14 diagram blocks, found {len(diagram_blocks)}"
    ...
    # 8. Check for Bracket Citation Leakage ([1], [121], [S1, p. 1])
    ...
    # 9. Check for internal generation terminology leaks
    ...
    print("=" * 80)
    print(f"[ALL TESTS PASSED] Form 4 Geography Topic 1 is fully verified and production-ready!")
