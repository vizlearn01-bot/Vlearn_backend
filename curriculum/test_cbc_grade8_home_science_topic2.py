"""
VLearn CBC Grade 8 Home Science — Automated Dual QA Test Suite for Topic 2: Consumer Education

Verifies:
  1. Hierarchy mapping: CBC -> Grade 8 (ID: 15) -> Home Science (ID: 28) -> Topic 2: Consumer Education
  2. Exact 2 Learning Units and 2 published Lessons
  3. Exact 16 Total Pages across both Lessons (8 pages per lesson)
  4. 100% presence of mandatory Card 1 visual hooks across both lessons
  ...
  7. Zero bracket citation leaks ([2], [3], [image_1]) across all blocks
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade8_home_science_topic2.py
"""
...
def test_cbc_grade8_home_science_topic2():
    print("=" * 80)
    print("RUNNING AUTOMATED DUAL QA TEST SUITE: CBC GRADE 8 HOME SCIENCE TOPIC 2 (CONSUMER EDUCATION)")
    print("=" * 80)
    ...
    assert len(units) == 2, f"Expected 2 Learning Units, found {len(units)}"
    ...
    assert len(lessons) == 2, f"Expected 2 Lessons, found {len(lessons)}"
    ...
    expected_pages = {
        1: 8, # Consumer Awareness & Household Buying Habits
        2: 8  # Market Competition & Consumer Protection
    }
    ...
    assert total_pages == 16, f"Total pages is {total_pages}, expected 16"
    print(f"[PASS] 5. Total Pages across all 2 Lessons: {total_pages} (Matches target: 16)")
    ...
    assert len(diagram_blocks) >= 6, f"Expected at least 6 diagram blocks, found {len(diagram_blocks)}"
    ...
    assert len(image_blocks) >= 2, f"Expected at least 2 image blocks, found {len(image_blocks)}"
    ...
    print("[PASS] 9. Zero bracket citations and zero developer meta-language leaks detected across all 32 blocks.")

    # 10. Check Knowledge Check Blocks
    qc_blocks = [b for b in all_blocks if b.block_type == "knowledge_check"]
    assert len(qc_blocks) >= 2, f"Expected at least 2 knowledge check blocks, found {len(qc_blocks)}"
    for qb in qc_blocks:
        c = qb.content or {}
    ...
    print("\n" + "=" * 80)
    print("ALL DUAL QA TESTS PASSED SUCCESSFULLY FOR CBC GRADE 8 HOME SCIENCE TOPIC 2!")
    print("=" * 80)

if __name__ == "__main__":
    test_cbc_grade8_home_science_topic2()
