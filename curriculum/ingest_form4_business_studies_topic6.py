"""
VLearn Form 4 Business Studies — Topic 6 Ingestion Engine (Economic Development and Planning)
...
from curriculum.topic6_data import TOPIC6_UNITS
...
@transaction.atomic
def ingest_topic6(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 6 INGESTION ENGINE (DEVELOPMENT & PLANNING)")
    print("=" * 80)
    ...
    # 2. Topic 6: Economic Development and Planning
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=6,
        defaults={
            "name": "Economic Development and Planning",
            "description": "Economic growth vs economic development, structural shifts, goals of development, characteristics of under-development, Vicious Cycle of Poverty, structural obstacles, development planning, and plan formulation vs implementation challenges."
        }
    )
    if topic.name != "Economic Development and Planning":
        topic.name = "Economic Development and Planning"
        topic.description = "Economic growth vs economic development, structural shifts, goals of development, characteristics of under-development, Vicious Cycle of Poverty, structural obstacles, development planning, and plan formulation vs implementation challenges."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 6...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC6_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 6 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L6U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 6 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 6")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 6 content")
    args = parser.parse_args()

    ingest_topic6(replace=args.replace)
