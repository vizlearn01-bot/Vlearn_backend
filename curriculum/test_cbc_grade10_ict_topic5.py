...
class TestCBCGrade10ICTTopic5(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 5 'Presentation Software' not found under ICT!"
    ...
    def test_01_hierarchy_integrity(self):
        """Verify the full curriculum hierarchy and Topic 5 configuration."""
        ...
        self.assertEqual(self.topic.name, "Presentation Software")
        self.assertTrue(self.topic.description, "Topic 5 missing description")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 5 lessons have Page 1 photographic visual hooks with Wikimedia URLs and learning goals."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 5 lessons contain sanitized, responsive vector SVGs attached as LessonAssets on Page 3."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_06_youtube_video_assets(self):
        ...
        print(f"[*] Topic 5 Video Coverage: {video_count}/{len(self.lessons)} ({coverage:.1f}%)")
    ...
    def test_08_formative_mcqs(self):
        ...
        self.assertGreaterEqual(mcq_count, 10, f"Expected >= 10 MCQs across Topic 5, found {mcq_count}")

    def test_09_pedagogical_page_completeness(self):
        """Verify the full 5-page pedagogical card architecture across all 5 lessons."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
