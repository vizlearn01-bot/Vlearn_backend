...
Verifies:
  1. Hierarchy resolution (CBC -> Grade 7 -> Home Science -> Topic 2: Small Kitchen Tools and Equipment)
  2. Exactly 3 Learning Units and 3 Published Lessons (status='published')
  3. Exactly 24 Total Structured Pages (8 pages per lesson)
  4. 100% Card 1 Visual Hooks presence with direct HTTP 200 responses
  5. 100% SVG validity, sanitization, and responsive dimensions (6 SVGs)
  6. Zero bracket citations ([47], [48], [49]) and zero developer meta-language leaks
  7. Standard markdown bullet lists (- ) with proper spacing
  8. 3 scenario knowledge checks with complete options, correct index, and pedagogical explanations
  9. Exactly 9 persisted LessonAsset records in database
"""
...
class TestCBCGrade7HomeScienceTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Small Kitchen Tools and Equipment").first()
    ...
    def test_01_hierarchy_resolution(self):
        """Test that Topic 2 resolves under CBC -> Grade 7 -> Home Science."""
        ...
        self.assertIsNotNone(self.topic, "Topic 'Small Kitchen Tools and Equipment' not found")
        self.assertEqual(self.topic.order, 2, "Topic order must be 2")
        print("\n[QA PASS] Hierarchy correctly resolved to Grade 7 Topic 2: Small Kitchen Tools (ID: 109).")

    def test_02_learning_units_and_published_lessons(self):
        """Test that exactly 3 Learning Units and 3 published lessons exist."""
        self.assertEqual(len(self.lessons), 3, f"Expected 3 lessons, found {len(self.lessons)}")
    ...
    def test_03_total_pages_distribution(self):
        """Test that each lesson has exactly 8 structured pages (24 total pages)."""
        total_pages = 0
    ...
    def test_04_card_1_visual_hooks_live_200(self):
        """Test that Card 1 of each of the 3 lessons has a tested HTTP 200 visual hook."""
        ...
        print("[QA PASS] All 3 Card 1 photographic hooks verified live with HTTP 200 responses.")

    def test_05_vector_svg_validity_and_dimensions(self):
        """Test that all 6 vector SVGs are valid, responsive, and attached."""
        svg_blocks = LessonBlock.objects.filter(lesson__in=self.lessons, block_type="suggested_diagram")
        self.assertEqual(svg_blocks.count(), 6, f"Expected 6 suggested_diagram blocks, found {svg_blocks.count()}")
        for b in svg_blocks:
        ...
        print("[QA PASS] All 6 Vector SVGs verified valid, responsive (800x450), and sanitized.")
    ...
    def test_06_zero_bracket_citations_and_meta_language(self):
        ...
        print("[QA PASS] Zero bracket citations and zero developer meta-language leaks detected across all 36 blocks.")
    ...
    def test_08_scenario_knowledge_checks(self):
        """Test that all 3 lessons end with a high-quality scenario knowledge check."""
        kc_blocks = LessonBlock.objects.filter(lesson__in=self.lessons, block_type="knowledge_check")
        self.assertEqual(kc_blocks.count(), 3, f"Expected 3 knowledge_check blocks, found {kc_blocks.count()}")
        for kc in kc_blocks:
            c = kc.content or {}
        ...
        print("[QA PASS] All 3 scenario knowledge checks validated with 4 options, valid index, and detailed explanation.")
    ...
    def test_09_total_assets_persisted(self):
        ...
        self.assertEqual(assets.count(), 9, f"Expected 9 persisted LessonAsset records, found {assets.count()}")
        img_assets = assets.filter(asset_type="image")
        diagram_assets = assets.filter(asset_type="diagram")
        self.assertEqual(img_assets.count(), 3, f"Expected 3 image assets, found {img_assets.count()}")
        self.assertEqual(diagram_assets.count(), 6, f"Expected 6 diagram assets, found {diagram_assets.count()}")
        print("[QA PASS] Exactly 9 LessonAsset records verified in database (3 photos + 6 diagrams).")
