...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 16: Marketing Agricultural Produce)
  ...
  4. Mandatory First-Card Visual Hooks (6 Tested Wikimedia URLs & attached LessonAssets)
  5. 6 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  ...
  8. Formative & Summative Scenario-Based MCQs Validation (13 MCQs with 4 Options, Valid Answer Key & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic16.py
"""
...
class TestCBCGrade10AgricultureTopic16(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Marketing Agricultural Produce").first()
        assert cls.topic, "Topic 16 'Marketing Agricultural Produce' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Marketing Agricultural Produce")
    ...
    def test_05_custom_vector_svgs(self):
        """Verify 6 custom responsive SVGs have viewBox, are sanitized, and have attached LessonAssets."""
    ...
    def test_08_formative_and_summative_mcqs(self):
        """Verify formative MCQs in Lessons 1-5 and Summative MCQs in Lesson 6 have 4 options, key, and explanation."""
        total_mcqs = 0
        for lesson in self.lessons:
        ...
        # Total MCQs = 5 (one per lesson 1-5) + 8 (summative in lesson 6) = 13 MCQs
        self.assertGreaterEqual(total_mcqs, 13, f"Expected at least 13 MCQs, found {total_mcqs}")
