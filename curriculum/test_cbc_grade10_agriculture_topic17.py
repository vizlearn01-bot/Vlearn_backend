...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 17: Composting)
  ...
  6. 2 Verified Educational YouTube Video Assets (Lesson 4 & Lesson 7)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic17.py
"""
...
class TestCBCGrade10AgricultureTopic17(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Composting").first()
        assert cls.topic, "Topic 17 'Composting' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.order, 17)
        self.assertEqual(self.topic.name, "Composting")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 4, 5, 6, 7, 9, 10]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video assets in Lesson 4 and Lesson 7."""
        for u_order in [4, 7]:
            lesson = next((l for l in self.lessons if l.learning_unit.order == u_order), None)
    ...
    def test_08_formative_and_summative_mcqs(self):
        ...
        # Total MCQs = 9 (one per lesson 1-9) + 8 (summative in lesson 10) = 17 MCQs (plus 1 on page 1 of lesson 10 if present) >= 17
