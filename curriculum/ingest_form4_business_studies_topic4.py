"""
VLearn Form 4 Business Studies — Topic 4 Ingestion Engine (Public Finance)
...
from curriculum.topic4_data import TOPIC4_UNITS


def clean_bracket_citations(text):
    """Strip bracketed citations like [253], [254], etc. from text."""
...
def ingest_topic4(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 4 INGESTION ENGINE (PUBLIC FINANCE)")
    print("=" * 80)
    ...
    # 2. Topic 4: Public Finance
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=4,
        defaults={
            "name": "Public Finance",
            "description": "Public finance, government revenue sources, recurrent and development expenditure, canons of public spending, taxation, direct vs indirect taxes, VAT, national budgets, and fiscal policy."
        }
    )
    if topic.name != "Public Finance":
        topic.name = "Public Finance"
        topic.description = "Public finance, government revenue sources, recurrent and development expenditure, canons of public spending, taxation, direct vs indirect taxes, VAT, national budgets, and fiscal policy."
        topic.save()
    ...
    for unit_idx, unit_data in enumerate(TOPIC4_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 4 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L4U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 4 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 4")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 4 content")
