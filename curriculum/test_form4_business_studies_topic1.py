"""
VLearn Form 4 Business Studies — Automated Test Suite for Topic 1: National Income

Verifies:
  1. Target Subject, Grade, Curriculum mapping (Business Studies, Form 4, 844)
  2. Topic 1 existence, name, and ordering
  3. Exact 8 Learning Units and 8 Lessons
  4. Exact 78 Pages across the 8 Lessons
  5. 100% presence of verified Wikimedia URLs and attributions on photographic blocks
  6. 100% presence of valid educational SVG diagrams
  7. Zero bracket citations ([298], [299], [1]) across all blocks
  8. Zero internal developer/generation metadata leaks
  9. 100% LessonAsset database linkage
  10. Verification of worked calculations and knowledge check questions

Usage:
  ./venv/bin/python curriculum/test_form4_business_studies_topic1.py
"""
...
def test_form4_business_studies_topic1():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 BUSINESS STUDIES TOPIC 1 (NATIONAL INCOME)")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="National Income").first()
    assert topic, "Topic 'National Income' not found!"
    ...
    expected_pages = {
        1: 10, # Meaning and Related Terms
        2: 10, # Circular Flow, Injections, Withdrawals
        3: 9,  # Equilibrium National Income
        4: 10, # Expenditure Approach
        5: 10, # Income Approach
        6: 10, # Output (Value-Added) Approach
        7: 9,  # National Income Statistics
        8: 10  # Factors & KCSE Applications
    }
    ...
    assert total_pages == 78, f"Total pages is {total_pages}, expected 78"
    print(f"[PASS] 5. Total Pages across all 8 Lessons: {total_pages} (Matches target: 78)")

    # 6. Check Wikimedia Photographic Assets
    image_blocks = [b for b in all_blocks if b.block_type == "suggested_image"]
    assert len(image_blocks) >= 6, f"Expected at least 6 suggested_image blocks, found {len(image_blocks)}"
    ...
    assert len(assets) >= 11, f"Expected at least 11 LessonAsset records, found {len(assets)}"
    for a in assets:
    ...
    # 9. Check for Bracket Citation Leakage ([298], [299], [1])
    ...
    assert len(worked_examples) >= 7, f"Expected at least 7 worked examples, found {len(worked_examples)}"
    assert len(knowledge_checks) >= 8, f"Expected at least 8 knowledge checks, found {len(knowledge_checks)}"
    assert len(comparison_tables) >= 4, f"Expected at least 4 comparison tables, found {len(comparison_tables)}"
    ...
    print("=" * 80)
    print("[ALL TESTS PASSED] Form 4 Business Studies Topic 1 is 100% verified and production-ready!")
    print("=" * 80)

if __name__ == "__main__":
    test_form4_business_studies_topic1()
