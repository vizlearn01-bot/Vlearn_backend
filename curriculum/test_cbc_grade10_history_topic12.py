...
from curriculum.ingest_cbc_grade10_history_topic12 import ingest_grade10_history_topic12


class TestCBCGrade10HistoryTopic12(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic is not None, "Topic 12 must exist in History Grade 10."
    ...
    def test_01_topic_hierarchy_metadata(self):
        ...
        self.assertIn("First Industrial Revolution and Africa", self.topic.name)
    ...
    def test_02_learning_units(self):
        ...
        expected_units = [
            (1, "Industrial Revolution in Britain and the USA"),
            (2, "Impacts on Africa"),
            (3, "Underdevelopment and African Responses"),
            (4, "Industrialisation and Colonisation")
        ]
    ...
    def test_04_pages_and_cards_structure(self):
        """Verify each lesson has between 4 and 6 distinct pages/cards."""
        total_pages = 0
        for unit in self.units:
            ...
            self.assertGreaterEqual(len(page_numbers), 4)
            self.assertLessEqual(len(page_numbers), 6)
            total_pages += len(page_numbers)
        self.assertGreaterEqual(total_pages, 19)
    ...
    def test_05_visualizations_and_svg_assets(self):
        ...
        self.assertGreaterEqual(len(svg_assets), 3)
