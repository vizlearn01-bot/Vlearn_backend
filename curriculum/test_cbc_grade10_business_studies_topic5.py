...
class TestCBCGrade10BusinessStudiesTopic5(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 5 'Business Activities' not found under Business Studies!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Business Activities")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 5 lessons have Page 1 photographic visual hooks with tested Wikimedia URLs."""
        for lesson in self.lessons:
    ...
    def test_06_youtube_video_assets(self):
        ...
        self.assertEqual(coverage, 100.0, f"Topic 5 video coverage not 100%: {coverage}%")

    def test_07_content_sanitization(self):
        """Verify zero bracket citations ([14], [29]), zero developer prompt leakage, and clean math."""
        for lesson in self.lessons:
            blocks = LessonBlock.objects.filter(lesson=lesson)
            for b in blocks:
                text_content = str(b.content) + " " + (b.title or "")
                # Check for raw citation brackets like [14], [228]
    ...
    def test_08_formative_mcqs(self):
        ...
        self.assertEqual(mcq_count, 10, f"Expected exactly 10 MCQs in Topic 5 (2 per lesson), found {mcq_count}")
