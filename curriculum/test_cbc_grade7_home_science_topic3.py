...
Verifies:
  1. Hierarchy resolution (CBC -> Grade 7 -> Home Science -> Topic 3: Cooking Food)
  ...
  6. Zero bracket citations ([58], [59], [60]) and zero developer meta-language leaks
...
class TestCBCGrade7HomeScienceTopic3(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Cooking Food").first()
    ...
    def test_01_hierarchy_resolution(self):
        """Test that Topic 3 resolves under CBC -> Grade 7 -> Home Science."""
        ...
        self.assertIsNotNone(self.topic, "Topic 'Cooking Food' not found")
        self.assertEqual(self.topic.order, 3, "Topic order must be 3")
        print("\n[QA PASS] Hierarchy correctly resolved to Grade 7 Topic 3: Cooking Food (ID: 110).")
