"""
Automated Quality Assurance Test Suite for CBC Grade 9 Agriculture — Topic 3: Integrated Farming
Curriculum: CBC -> Grade 9 -> Agriculture -> Topic 3: Integrated Farming

Test Coverage:
  1. Hierarchy and Topic Configuration Integrity (Topic Order: 3)
  2. Exactly 11 LearningUnits and 11 Published Lessons
  3. Dynamic Page and Block Counts & Structural Consistency
  ...
  6. Topic Video Review (YouTube ID: uFnDdYWgkV8, verified status, attached LessonAsset)
  7. Zero Citation Bracket Leaks, Zero Developer Meta-Tags, Zero Raw LaTeX
  8. Formative (11) and Summative (10) MCQ Validation (21 Total Knowledge Checks)

Run:
  ./venv/bin/python curriculum/test_cbc_grade9_agriculture_topic3.py
"""
...
class TestCBCGrade9AgricultureTopic3(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic_name = "Integrated Farming"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertIsNotNone(self.grade, "Grade 'Grade 9' must exist under CBC")
        self.assertEqual(self.grade.level, 9, "Grade 9 level must be 9")
        self.assertIsNotNone(self.subject, "Subject 'Agriculture' must exist under Grade 9")
    ...
    def test_02_units_and_published_lessons_count(self):
        """Verify exactly 11 LearningUnits and 11 published Lessons exist in sequential order."""
        units = LearningUnit.objects.filter(topic=self.topic).order_by("order")
        self.assertEqual(units.count(), 11, f"Expected exactly 11 LearningUnits, found {units.count()}")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 11 lessons have Page 1 photographic visual hooks with HTTP 200 URLs & attached LessonAssets."""
    ...
    def test_05_custom_vector_svgs_sanitization(self):
        """Verify all 11 lessons have sanitized responsive vector SVGs attached."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_07_zero_citation_and_meta_leaks(self):
        ...
        for block in blocks:
            ...
            # Check content (excluding SVG raw tags)
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all 21 interactive knowledge_check blocks across the 11 lessons."""
        mcq_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        self.assertEqual(mcq_blocks.count(), 21, f"Expected 21 MCQs (11 formative + 10 summative), found {mcq_blocks.count()}")
