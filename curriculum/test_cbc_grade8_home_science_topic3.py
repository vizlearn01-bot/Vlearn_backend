"""
VLearn CBC Grade 8 Home Science — Automated Dual QA Test Suite for Topic 3: Textile and Clothing

Verifies:
  1. Hierarchy mapping: CBC -> Grade 8 (ID: 15) -> Home Science (ID: 28) -> Topic 3: Textile and Clothing
  2. Exact 3 Learning Units and 3 published Lessons
  3. Exact 24 Total Pages across all 3 Lessons (8 pages per lesson)
  4. 100% presence of mandatory Card 1 visual hooks across all 3 lessons
  5. 100% presence of valid, sanitized SVGs on all suggested_diagram blocks (8 SVGs)
  6. 100% presence of verified photographic assets with full attribution on all suggested_image blocks (3 photos)
  7. Zero bracket citation leaks ([6], [7], [76]) across all blocks
  8. Zero developer meta-language leaks in titles, content, or captions
  9. Valid knowledge check configurations with questions, options, and explanations across all 3 lessons

Usage:
  ./venv/bin/python curriculum/test_cbc_grade8_home_science_topic3.py
"""
...
def test_cbc_grade8_home_science_topic3():
    print("=" * 80)
    print("RUNNING AUTOMATED DUAL QA TEST SUITE: CBC GRADE 8 HOME SCIENCE TOPIC 3 (TEXTILE AND CLOTHING)")
    print("=" * 80)
    ...
    assert len(units) == 3, f"Expected 3 Learning Units, found {len(units)}"
    ...
    expected_pages = {
        1: 8, # Artificial Textile Fibres
        2: 8, # Seams in Garment Construction
        3: 8  # Methods of Controlling Fullness
    }
    ...
    assert total_pages == 24, f"Total pages is {total_pages}, expected 24"
    print(f"[PASS] 5. Total Pages across all 3 Lessons: {total_pages} (Matches target: 24)")
    ...
    assert len(diagram_blocks) >= 8, f"Expected at least 8 diagram blocks, found {len(diagram_blocks)}"
    ...
    assert len(image_blocks) >= 3, f"Expected at least 3 image blocks, found {len(image_blocks)}"
    ...
    meta_terms = [
        ...
        "system prompt", "developer instruction", "generation rule", "lessons.md", "lessons2.md", "blueprint component"
    ]
    ...
    print("[PASS] 9. Zero bracket citations and zero developer meta-language leaks detected across all blocks.")

    # 10. Check Knowledge Check Blocks
    qc_blocks = [b for b in all_blocks if b.block_type == "knowledge_check"]
    assert len(qc_blocks) == 3, f"Expected 3 knowledge check blocks, found {len(qc_blocks)}"
    for qb in qc_blocks:
        c = qb.content or {}
    ...
    print("\n" + "=" * 80)
    print("ALL DUAL QA TESTS PASSED SUCCESSFULLY FOR CBC GRADE 8 HOME SCIENCE TOPIC 3!")
    print("=" * 80)

if __name__ == "__main__":
    test_cbc_grade8_home_science_topic3()
