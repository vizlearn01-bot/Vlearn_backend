...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 15: Establishing an Agricultural Enterprise)
  ...
  4. Mandatory First-Card Visual Hooks (13 Tested Wikimedia URLs & attached LessonAssets)
  5. 8 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. 1 Verified Educational YouTube Video Asset (Lesson 1)
  7. Content Sanitization (0 Bracket Citations, 0 Developer Meta-Terms, 0 Raw LaTeX Leaks)
  8. Formative & Summative Scenario-Based MCQs Validation (20 MCQs with 4 Options, Valid Answer Key & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic15.py
"""
...
class TestCBCGrade10AgricultureTopic15(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Establishing an Agricultural Enterprise").first()
        assert cls.topic, "Topic 15 'Establishing an Agricultural Enterprise' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Establishing an Agricultural Enterprise")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 13 lessons have Page 1 photographic visual hooks with HTTP 200 URLs & attached LessonAssets."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 3, 4, 5, 7, 8, 12, 13]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video asset in Lesson 1."""
        lesson = next((l for l in self.lessons if l.learning_unit.order == 1), None)
        video_block = LessonBlock.objects.filter(
            lesson=lesson,
            block_type="suggested_video"
        ).first()
        self.assertIsNotNone(video_block, "Lesson 1 missing suggested_video block!")

        content = video_block.content or {}
        v_url = content.get("url")
        self.assertTrue(v_url and "youtube.com" in v_url, f"Lesson 1 video URL invalid: {v_url}")

        asset = video_block.assets.filter(asset_type="youtube").first()
        self.assertIsNotNone(asset, "Lesson 1 video missing attached LessonAsset!")
        self.assertEqual(asset.status, "attached")
    ...
    def test_08_formative_and_summative_mcqs(self):
        """Verify formative MCQs in Lessons 1-12 and Summative MCQs in Lesson 13 have 4 options, key, and explanation."""
        total_mcqs = 0
        for lesson in self.lessons:
        ...
        # Total MCQs = 12 (one per lesson 1-12) + 8 (summative in lesson 13) = 20 MCQs
        self.assertGreaterEqual(total_mcqs, 20, f"Expected at least 20 MCQs, found {total_mcqs}")
