"""
VLearn Form 4 Business Studies — Topic 7 Ingestion Engine (International Trade)
...
from curriculum.topic7_data import TOPIC7_UNITS
...
@transaction.atomic
def ingest_topic7(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 7 INGESTION ENGINE (INTERNATIONAL TRADE)")
    print("=" * 80)
    ...
    # 2. Topic 7: International Trade
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=7,
        defaults={
            "name": "International Trade",
            "description": "Bilateral and multilateral trade, home trade vs foreign trade, Terms of Trade (TOT), Balance of Payments (BOP) accounts and deficit correction, Terms of Sale, trade documents, international financial institutions (IMF, World Bank, ADB), trade protectionism, exchange rates, and Export Processing Zones (EPZs)."
        }
    )
    if topic.name != "International Trade":
        topic.name = "International Trade"
        topic.description = "Bilateral and multilateral trade, home trade vs foreign trade, Terms of Trade (TOT), Balance of Payments (BOP) accounts and deficit correction, Terms of Sale, trade documents, international financial institutions (IMF, World Bank, ADB), trade protectionism, exchange rates, and Export Processing Zones (EPZs)."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 7...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC7_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 7 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L7U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 7 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 7")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 7 content")
    args = parser.parse_args()

    ingest_topic7(replace=args.replace)
