"""
VLearn Automated QA Suite: Grade 10 Aviation — Topic 2
Safety in Aviation
"""
...
class TestGrade10AviationTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.subject = Subject.objects.get(id=44)
        cls.topic = Topic.objects.get(id=248, subject=cls.subject)
    ...
    def test_topic_hierarchy(self):
        """Verify Topic 2 exists with correct attributes."""
        self.assertEqual(self.topic.name, "Safety in Aviation")
        self.assertEqual(self.topic.order, 1)
        self.assertEqual(self.units.count(), 6, "Expected 6 LearningUnits under Topic 2")
        self.assertEqual(self.lessons.count(), 6, "Expected 6 Lessons under Topic 2")
