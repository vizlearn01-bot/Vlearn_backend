"""
Test: CBC Grade 8 Home Science — Comprehensive Video Enrichment Verification

Asserts that:
1. All 20 lessons in Grade 8 Home Science have a valid suggested_video block on Page 9.
...
def run_tests():
    print("=" * 80)
    print("[TEST] CBC Grade 8 Home Science — Video Enrichment Verification")
    print("=" * 80)
    ...
    for topic in subject.topics.order_by("order"):
        ...
        for unit in topic.learning_units.order_by("order"):
            for lesson in unit.lessons.all():
                ...
                if not video_block:
                    print(f"  [FAIL] Lesson {lesson.id}: '{lesson.title}' — Missing suggested_video block on Page 9")
                    failed += 1
                    continue

                url = video_block.content.get("url", "")
                if not url or ("youtube.com" not in url and "youtu.be" not in url):
                    print(f"  [FAIL] Lesson {lesson.id}: '{lesson.title}' — Invalid YouTube URL in video block: {url}")
                    failed += 1
                    continue
                ...
                if not callout_block:
                    print(f"  [FAIL] Lesson {lesson.id}: '{lesson.title}' — Missing reflection callout on Page 9")
                    failed += 1
                    continue
                ...
                if not video_asset:
                    print(f"  [FAIL] Lesson {lesson.id}: '{lesson.title}' — Missing attached video LessonAsset")
                    failed += 1
                    continue
                ...
                if p1_to_8_blocks < 8:
                    print(f"  [FAIL] Lesson {lesson.id}: '{lesson.title}' — Expected >=8 blocks on pages 1-8, found {p1_to_8_blocks}")
                    failed += 1
                    continue

                print(f"  [PASS] Lesson {lesson.id}: '{lesson.title[:45]}'")
                print(f"         Video: {url} | Asset: OK | Pages 1-8 intact ({p1_to_8_blocks} blocks)")
                passed += 1

    print("\n" + "=" * 80)
    if failed == 0:
        print(f"[ALL TESTS PASSED] {passed}/{total_lessons} Grade 8 lessons verified successfully.")
    else:
