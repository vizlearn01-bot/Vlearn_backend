"""
VLearn Form 4 History — Automated Test Suite for Topic 8: Public Revenue and Expenditure in Kenya

Verifies:
  1. Target Subject, Grade, Curriculum mapping (History, Form 4, 844)
  2. Topic 8 existence and ordering (Order: 8)
  3. Exact 5 Learning Units and 5 Lessons
  4. Exact 81 Pages across the 5 Lessons (15, 16, 16, 16, 18)
  ...
  7. Zero bracket citations across all blocks
  8. Zero internal developer/generation metadata leaks
  9. 100% LessonAsset associations (Images + Videos)

Usage:
  ./venv/bin/python curriculum/test_form4_history_topic8.py
"""
...
def test_form4_history_topic8():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 HISTORY TOPIC 8 (PUBLIC REVENUE AND EXPENDITURE)")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="Public Revenue and Expenditure in Kenya").first()
    assert topic, "Topic 8 not found!"
    ...
    print("[PASS] 4.1 Every single lesson in Topic 8 has an establishing real-world visual on Page 1.")
    ...
    expected_pages = {
        1: 15, # Foundations of Public Finance: Meaning, Historical Evolution, and Constitutional Principles (Article 201)
        2: 16, # The National Budget: Meaning, Strategic Importance, and Step-by-Step Preparation Cycle
        3: 16, # Sources of Public Revenue: Direct and Indirect Taxes, Non-Tax Revenues, External Aid, and County Finances
        4: 16, # National Public Funds, Equitable Sharing Criteria (Article 203), and Expenditure Classification
        5: 18  # Public Finance Management: Oversight Controls, Key Institutions (CRA, COB, Auditor-General, CBK), and Synthesis
    }
    ...
    assert total_pages == 81, f"Total pages is {total_pages}, expected 81"
    print(f"[PASS] 5. Total Pages across all 5 Lessons: {total_pages} (Matches target: 81)")

    # 6. Check Wikimedia Photographic Assets
    image_blocks = [b for b in all_blocks if b.block_type == "suggested_image"]
    assert len(image_blocks) >= 3, f"Expected at least 3 suggested_image blocks, found {len(image_blocks)}"
    ...
    assert len(assets) >= 8, f"Expected at least 8 LessonAsset records, found {len(assets)}"
    for a in assets:
    ...
    # 9. Check for Bracket Citation Leakage ([87], [244], [1])
    ...
    meta_phrases = ["Visual Representation:", "Building Intuition:", "AI-generated", "AI Generated", "[VISUAL OPPORTUNITY]", "[VIDEO OPPORTUNITY]"]
    meta_leaks = []
    for b in all_blocks:
        b_title = b.title or ""
    ...
    print("=" * 80)
    print(f"[ALL TESTS PASSED] Form 4 History Topic 8 is fully verified, robust, and production-ready!")
