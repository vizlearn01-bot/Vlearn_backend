"""
VLearn Form 4 Business Studies — Topic 11 Ingestion Engine (Ledger)
...
from curriculum.topic11_data import TOPIC11_UNITS
...
@transaction.atomic
def ingest_topic11(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 11 INGESTION ENGINE (LEDGER)")
    print("=" * 80)
    ...
    topic, _ = Topic.objects.get_or_create(
        ...
        defaults={
            "name": "Ledger",
            "description": "Meaning and purpose of a ledger, T-account structure, double-entry rules for assets, liabilities, capital, expenses, and revenues, specialized stock accounts (purchases, sales, returns inwards/outwards), 5 steps of balancing off accounts (Balance c/d and Balance b/d), 6 ledger classifications (Sales, Purchase, Cash Book, Nominal, Private, General), and Trial Balance error analysis."
        }
    )
    if topic.name != "Ledger":
        topic.name = "Ledger"
        topic.description = "Meaning and purpose of a ledger, T-account structure, double-entry rules for assets, liabilities, capital, expenses, and revenues, specialized stock accounts (purchases, sales, returns inwards/outwards), 5 steps of balancing off accounts (Balance c/d and Balance b/d), 6 ledger classifications (Sales, Purchase, Cash Book, Nominal, Private, General), and Trial Balance error analysis."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 11...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC11_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 11 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L11U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 11 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 11")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 11 content")
    args = parser.parse_args()

    ingest_topic11(replace=args.replace)
