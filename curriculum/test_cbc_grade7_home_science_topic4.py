"""
VLearn CBC Grade 7 Home Science — Topic 4: Consumer Education
Dual QA Test Suite (Pedagogical Quality & Platform Integrity)

Verifies:
  1. Hierarchy resolution (CBC -> Grade 7 -> Home Science -> Topic 4: Consumer Education)
  ...
  6. Zero bracket citations ([70], [71], [72]) and zero developer meta-language leaks
...
class TestCBCGrade7HomeScienceTopic4(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Consumer Education").first()
    ...
    def test_01_hierarchy_resolution(self):
        """Test that Topic 4 resolves under CBC -> Grade 7 -> Home Science."""
        ...
        self.assertIsNotNone(self.topic, "Topic 'Consumer Education' not found")
        self.assertEqual(self.topic.order, 4, "Topic order must be 4")
        print("\n[QA PASS] Hierarchy correctly resolved to Grade 7 Topic 4: Consumer Education (ID: 111).")
