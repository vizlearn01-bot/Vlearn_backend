"""
VLearn CBC Grade 10 History — Topic 9: Modern Nationalism in Africa
...
from curriculum.ingest_cbc_grade10_history_topic9 import ingest_grade10_history_topic9


class TestCBCGrade10HistoryTopic9(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ingest_grade10_history_topic9(replace=True)
        ...
        assert cls.topic is not None, "Topic 9 must exist in History Grade 10."
    ...
    def test_01_topic_hierarchy_metadata(self):
        ...
        self.assertIn("Modern Nationalism in Africa", self.topic.name)
        self.assertTrue(len(self.topic.description) > 20)

    def test_02_learning_units(self):
        """Verify all 5 learning units exist with correct titles and orders."""
        self.assertEqual(len(self.units), 5, f"Expected 5 units, found {len(self.units)}")
        expected_units = [
            (1, "Meaning and Historical Roots of African Nationalism"),
            (2, "Key Leaders and Ideas: Models of African Agency"),
            (3, "Best Practices to Strengthen Nationalism and National Cohesion"),
            (4, "Nationalism, Development, and Globalisation"),
            (5, "Integrated Inquiry: The African Nationalism Research Lab")
        ]
    ...
    def test_03_lessons_published_and_metadata(self):
        """Verify all 5 lessons are published with version=1 and valid metadata."""
    ...
    def test_04_pages_and_cards_structure(self):
        """Verify each lesson has between 5 and 7 distinct pages/cards."""
        total_pages = 0
        for unit in self.units:
            ...
            self.assertGreaterEqual(len(page_numbers), 5)
            self.assertLessEqual(len(page_numbers), 7)
            total_pages += len(page_numbers)
        self.assertGreaterEqual(total_pages, 25)
    ...
    def test_05_visualizations_and_svg_assets(self):
        ...
        self.assertGreaterEqual(len(svg_assets), 4)
    ...
    def test_07_knowledge_check_mcqs(self):
        ...
        self.assertGreaterEqual(blocks.count(), 8)
        for b in blocks:
