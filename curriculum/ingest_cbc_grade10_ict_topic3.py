...
from curriculum.cbc_grade10_ict_topic3_data import TOPIC_3_LESSONS

def clean_text(text: str) -> str:
    """Removes bracket citations, normalizes unicode bullets into standard markdown list items."""
    if not text:
        return ""
...
def ingest_grade10_ict_topic3(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 3: Operating Systems (Grade 10 ICT)")
    print("=" * 80)

    curriculum = Curriculum.objects.filter(name__iexact="CBC").first() or Curriculum.objects.filter(id=5).first()
    assert curriculum, "Curriculum 'CBC' not found in database!"
    ...
    subject, _ = Subject.objects.get_or_create(
        grade=grade,
        name="ICT",
        defaults={"description": "Senior Secondary Information and Communication Technology Curriculum (Grade 10 CBC)"}
    )
    ...
    topic, t_created = Topic.objects.get_or_create(
        ...
        defaults={
            ...
            "description": "Understanding operating system purpose, core functions, platform types, user interfaces, file systems, and security controls."
        }
    )
    if not t_created:
        topic.name = "Operating Systems"
        topic.description = "Understanding operating system purpose, core functions, platform types, user interfaces, file systems, and security controls."
        topic.save()
        ...
        for item in TOPIC_3_LESSONS:
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    raw_content = block_def.get("content", {})
                    
                    # Clean content dictionary except for svg_content which needs preserving verbatim
                    if isinstance(raw_content, dict) and "svg_content" in raw_content:
                        svg_payload = raw_content["svg_content"]
                        other_content = {k: v for k, v in raw_content.items() if k != "svg_content"}
                        cleaned = clean_dict(other_content)
                        cleaned["svg_content"] = svg_payload
                        b_content = cleaned
                    else:
                        b_content = clean_dict(raw_content)

                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_ict_t3_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 3 INGESTION COMPLETE:")
...
if __name__ == "__main__":
    ingest_grade10_ict_topic3(replace=True)
