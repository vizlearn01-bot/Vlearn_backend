"""
VLearn Form 4 Agriculture — Topic 1: Livestock Production V (Poultry)
...
Usage:
  ./venv/bin/python curriculum/ingest_form4_agriculture_topic1.py [--replace]
"""
...
from curriculum.topic1_agriculture_data import ALL_LESSONS

def clean_text(raw_str):
    """Remove source citation brackets like [1], [298], [1, 2] and normalize whitespace."""
...
def run_ingestion(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 AGRICULTURE TOPIC 1 INGESTION ENGINE")
    print("=" * 80)
    ...
    with transaction.atomic():
        ...
        topic, _ = Topic.objects.get_or_create(
            subject=subject,
            name="Livestock Production V (Poultry)",
            defaults={
                "order": 1,
                "description": (
                    "Covers egg structure, candling, incubation, brooding, "
                    "grower/layer/broiler management, rearing systems, vices, and marketing."
                )
            }
        )
        topic.order = 1
            ...
            for page_idx, page in enumerate(pages, 1):
                ...
                for comp_idx, block_info in enumerate(blocks, 1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"ag4_t1_l{unit_order}_p{page_num}_b{comp_idx}",
                        block_type=b_type,
        ...
        print("\n" + "=" * 80)
        print(f"FORM 4 AGRICULTURE TOPIC 1 INGESTION COMPLETE")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Agriculture Topic 1")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 1 data")
