...
Validates:
  1. Curriculum Hierarchy (CBC -> Grade 10 -> Biology -> Topic 2)
  2. Learning Units & Published Lessons 1-to-1 Parity (3 Comprehensive Lessons)
  3. Concept Card Distribution & Page Numbering (35 Total Cards)
  4. Block Integrity & Typing (Learning Goals, Explanations, Tables, Steps, Knowledge Checks)
  5. Visual Enrichment Assets (4 High-Quality SVGs, 3 Contextualized Photos, 3 YouTube Videos - 1 per lesson)
  6. Assessment Integrity (Valid Questions, Options, Answer Keys, Explanations)
  7. Content Leak Audit (Zero [VISUAL: ...] tags, zero citation brackets [134], zero corrupted LaTeX)

Usage:
  ./venv/bin/python curriculum/test_grade10_biology_topic2.py
"""
...
def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED QA: GRADE 10 BIOLOGY TOPIC 2 (ANATOMY & PHYSIOLOGY)")
    print("=" * 80)
    ...
    if not topic:
        errors.append("Topic 'Anatomy and Physiology of Plants' not found under Grade 10 Biology!")
    ...
    # 2. Units & Lessons Count (Expected: 3 Comprehensive Lessons)
    units = list(topic.learning_units.all().order_by("order"))
    lessons = list(topic.lessons.all().order_by("learning_unit__order"))

    if len(units) != 3:
        errors.append(f"Expected 3 LearningUnits, found {len(units)}")
    else:
        print(f"[PASS] Learning Units Count: {len(units)} (1-to-1 verified)")

    if len(lessons) != 3:
        errors.append(f"Expected 3 Published Lessons, found {len(lessons)}")
    else:
    ...
    for idx, (unit, lesson) in enumerate(zip(units, lessons), 1):
        ...
        if page_count < 10:
            warnings.append(f"Lesson {idx} ({lesson.title}) has {page_count} pages (expected >= 10)")
    ...
    print(f"Total Wikimedia Photos: {total_photos} blocks, {photo_assets} LessonAsset records (Expected: 3)")
    print(f"Total YouTube Videos:   {total_videos} blocks, {video_assets} LessonAsset records (Expected: 3)")
    print(f"Total LessonAssets:     {lesson_assets.count()} (Expected: 10)")

    if diagram_assets != 4:
        errors.append(f"Expected 4 diagram assets, found {diagram_assets}")
    if photo_assets != 3:
        errors.append(f"Expected 3 photo assets, found {photo_assets}")
    if video_assets != 3:
        errors.append(f"Expected 3 video assets, found {video_assets}")
    ...
    leak_patterns = [
        ...
        (r'\[(?:\d+|image_\d+)\]', "Raw Bracket Citation Leak [e.g. 134]"),
    ...
    print("\n[SUCCESS] ALL GRADE 10 BIOLOGY TOPIC 2 TESTS PASSED!")
    return True
