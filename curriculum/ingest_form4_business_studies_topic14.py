"""
VLearn Form 4 Business Studies — Topic 14 Ingestion Engine (Financial Statements)
...
from curriculum.topic14_data import TOPIC14_UNITS
...
@transaction.atomic
def ingest_topic14(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 14 INGESTION ENGINE (FINANCIAL STATEMENTS)")
    print("=" * 80)
    ...
    # 2. Topic 14: Financial Statements
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=14,
        defaults={
            "name": "Financial Statements",
            "description": "Purpose and components of financial statements, stakeholder decision needs, the Trading Account (Net Sales, Net Purchases, COGS, Gross Profit), the Profit and Loss Account (Net Profit, operating expenses, non-trading revenues), the Balance Sheet (Fixed vs Current Assets, Capital equation, Long-term & Current Liabilities), Working Capital and Capital Employed ratio analysis, year-end Accruals and Prepayments adjustments, complete Nyaituya 5-adjustment walkthrough, and KCSE essay mastery."
        }
    )
    if topic.name != "Financial Statements":
        topic.name = "Financial Statements"
        topic.description = "Purpose and components of financial statements, stakeholder decision needs, the Trading Account (Net Sales, Net Purchases, COGS, Gross Profit), the Profit and Loss Account (Net Profit, operating expenses, non-trading revenues), the Balance Sheet (Fixed vs Current Assets, Capital equation, Long-term & Current Liabilities), Working Capital and Capital Employed ratio analysis, year-end Accruals and Prepayments adjustments, complete Nyaituya 5-adjustment walkthrough, and KCSE essay mastery."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 14...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC14_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 14 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L14U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 14 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 14")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 14 content")
    args = parser.parse_args()

    ingest_topic14(replace=args.replace)
