"""
VLearn Form 4 Business Studies — Topic 10 Ingestion Engine (Business Transactions)
...
from curriculum.topic10_data import TOPIC10_UNITS
...
@transaction.atomic
def ingest_topic10(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 10 INGESTION ENGINE (BUSINESS TRANSACTIONS)")
    print("=" * 80)
    ...
    # 2. Topic 10: Business Transactions
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=10,
        defaults={
            "name": "Business Transactions",
            "description": "Definition and classification of business transactions into cash and credit categories, modes of payment, 3 balance sheet adjustment rules (asset swap, liability swap, dual expansion/contraction), Bigfoot Communications 7-step case study, 4 causes of capital changes (drawings, investment, profit, loss), and the Capital Tracking Equation (CC = OC + P + I - D)."
        }
    )
    if topic.name != "Business Transactions":
        topic.name = "Business Transactions"
        topic.description = "Definition and classification of business transactions into cash and credit categories, modes of payment, 3 balance sheet adjustment rules (asset swap, liability swap, dual expansion/contraction), Bigfoot Communications 7-step case study, 4 causes of capital changes (drawings, investment, profit, loss), and the Capital Tracking Equation (CC = OC + P + I - D)."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 10...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC10_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 10 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L10U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 10 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 10")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 10 content")
    args = parser.parse_args()

    ingest_topic10(replace=args.replace)
