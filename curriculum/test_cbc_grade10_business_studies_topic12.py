...
class TestCBCGrade10BusinessStudiesTopic12(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 12 'International Trade' not found in database!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "International Trade")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 8, f"Expected 8 units in Topic 12, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 8, f"Expected 8 lessons in Topic 12, found {len(self.lessons)}")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 8 lessons have Page 1 photographic visual hooks with valid Wikimedia URLs."""
        for lesson in self.lessons:
    ...
    def test_06_youtube_video_assets(self):
        """Verify 100% YouTube video coverage across all 8 lessons."""
        video_count = 0
        for lesson in self.lessons:
            ...
            if yt_asset or yt_block:
                video_count += 1
        self.assertEqual(video_count, len(self.lessons), f"Expected 100% video coverage in Topic 12, found {video_count}/{len(self.lessons)}")
