...
from curriculum.ingest_cbc_grade10_history_topic10 import ingest_grade10_history_topic10


class TestCBCGrade10HistoryTopic10(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ingest_grade10_history_topic10(replace=True)
        ...
        assert cls.topic is not None, "Topic 10 must exist in History Grade 10."
    ...
    def test_01_topic_hierarchy_metadata(self):
        ...
        self.assertIn("French Revolution", self.topic.name)
    ...
    def test_02_learning_units(self):
        ...
        expected_units = [
            (1, "Context and Causes of the Revolution"),
            (2, "Course and Turning Points of the Revolution"),
            (3, "Consequences and Global Legacies of the French Revolution"),
            (4, "Why the French Revolution Matters Today")
        ]
    ...
    def test_03_lessons_published_and_metadata(self):
        """Verify all 4 lessons are published with version=1 and valid metadata."""
    ...
    def test_04_pages_and_cards_structure(self):
        """Verify each lesson has between 5 and 8 distinct pages/cards."""
        total_pages = 0
        for unit in self.units:
            ...
            self.assertLessEqual(len(page_numbers), 8)
            total_pages += len(page_numbers)
        self.assertGreaterEqual(total_pages, 24)
