...
CBC Grade 7 Home Science — Topic 9: Special Treatments in Laundrywork

Validates:
1. Hierarchy Resolution (CBC -> Grade 7 -> Home Science -> Topic 9: Special Treatments in Laundrywork, ID/Order: 9)
...
class TestCBCGrade7HomeScienceTopic9(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Special Treatments in Laundrywork").first()
    ...
    def test_01_hierarchy_resolution(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 9 (Special Treatments in Laundrywork) missing!")
        self.assertEqual(self.topic.order, 9, f"Expected Topic 9 order to be 9, got {self.topic.order}")
        print(f"\n[QA PASS] Hierarchy correctly resolved to Grade 7 Topic 9: {self.topic.name} (ID: {self.topic.id}).")
