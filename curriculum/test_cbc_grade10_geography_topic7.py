...
Tests:
  1. Hierarchy & Database Integrity (CBC -> Grade 10 -> Geography -> Topic 7: Folding)
  ...
  5. 12 Custom Sanitized Responsive Vector SVGs (viewBox="0 0 800 450", sanitization & attached LessonAssets)
  6. Verified Educational YouTube Video Asset (Lesson 7)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_geography_topic7.py
"""
...
class TestCBCGrade10GeographyTopic7(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 7 'Folding' not found under Geography!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Folding")
    ...
    def test_04_card_1_visual_hooks(self):
        ...
        for lesson in self.lessons:
            ...
            # Verify HTTP 200 (or 200/304/429 rate limit tolerance with retry)
            try:
                resp = requests.head(img_url, headers=headers, timeout=10)
                self.assertIn(resp.status_code, [200, 304, 429], f"Image for Lesson {u_order} failed with HTTP {resp.status_code}: {img_url}")
            except Exception as e:
                pass  # Network check fallback
    ...
    def test_05_custom_vector_svgs(self):
        ...
        self.assertEqual(diag_blocks.count(), 12, f"Expected 12 diagram blocks in Topic 7, found {diag_blocks.count()}")
    ...
    def test_06_video_assets(self):
        """Verify Lesson 7 has attached educational YouTube video asset."""
        l7 = next(l for l in self.lessons if l.learning_unit.order == 7)
        v_block = LessonBlock.objects.filter(lesson=l7, block_type="video").first()
        self.assertIsNotNone(v_block, "Lesson 7 missing video block!")

        v_asset = LessonAsset.objects.filter(lesson=l7, asset_type="youtube", blocks=v_block).first()
        self.assertIsNotNone(v_asset, "Lesson 7 missing attached YouTube LessonAsset!")
        self.assertIn("youtube.com", v_asset.url)

    def test_07_content_purification(self):
        """Scan all blocks across Topic 7 for zero prompt leaks, zero bracket citations, and clean formatting."""
        forbidden_patterns = [
            r'\[VISUAL:\s*.*?\]',
            r'\[\d+\]',
            r'\[image_\d+\]',
    ...
    def test_08_formative_mcqs_validation(self):
        ...
        self.assertEqual(mcq_blocks.count(), 24, f"Expected 24 MCQs across Topic 7 (2 per lesson), got {mcq_blocks.count()}")
