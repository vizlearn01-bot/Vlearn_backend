"""
VLearn Form 4 Business Studies — Automated Test Suite: Topic 13 (Cash Book)
"""
...
def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 BUSINESS STUDIES TOPIC 13 (CASH BOOK)")
    print("=" * 80)
    ...
    assert topic is not None, "Topic 13 (Cash Book) not found!"
    assert topic.name == "Cash Book", f"Unexpected topic name: {topic.name}"
    ...
    assert units.count() == 4, f"Expected 4 Learning Units, found {units.count()}"
    
    print(f"[PASS] 3. Found exactly 4 Learning Units:")
    for u in units:
    ...
    print(f"[PASS] 4. Found exactly 4 Published Lessons:")
    for l in lessons:
    ...
    assert total_pages == 40, f"Total pages across topic is {total_pages}, expected 40!"
    print(f"[PASS] 5. Total Pages across all 4 Lessons: {total_pages} (Matches target: 40)")
    ...
    print("=" * 80)
    print("[ALL TESTS PASSED] Form 4 Business Studies Topic 13 is 100% verified and production-ready!")
    print("=" * 80)
