...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 5: Growing Selected Crops)
  ...
  6. 2 Verified Educational YouTube Video Assets (Lesson 3 & Lesson 6)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic5.py
"""
...
class TestCBCGrade10AgricultureTopic5(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Growing Selected Crops").first()
        assert cls.topic, "Topic 5 'Growing Selected Crops' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Growing Selected Crops")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 3, 4, 5, 6, 8, 10]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video assets in Lessons 3 and 6."""
        for u_order in [3, 6]:
            lesson = next((l for l in self.lessons if l.learning_unit.order == u_order), None)
