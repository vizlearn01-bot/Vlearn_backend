...
class TestCBCGrade10BusinessStudiesTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 2 'Business Goals' not found under Business Studies!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Business Goals")
    ...
    def test_03_card_and_block_structure(self):
        """Verify every lesson has valid page counts (>= 5 pages) and non-empty blocks."""
    ...
    def test_06_youtube_video_assets(self):
        ...
        self.assertGreaterEqual(coverage, 80.0, f"Topic 2 video coverage below 80%: {coverage}%")

    def test_07_content_sanitization(self):
        """Verify zero bracket citations ([310], [1]), zero developer prompt leakage, and clean math."""
        for lesson in self.lessons:
            blocks = LessonBlock.objects.filter(lesson=lesson)
            for b in blocks:
                text_content = str(b.content) + " " + (b.title or "")
                # Check for raw citation brackets like [310], [12]
    ...
    def test_08_formative_mcqs(self):
        ...
        self.assertGreaterEqual(mcq_count, 12, f"Expected >= 12 MCQs in Topic 2, found {mcq_count}")
