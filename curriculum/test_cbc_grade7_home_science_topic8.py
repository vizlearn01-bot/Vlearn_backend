...
CBC Grade 7 Home Science — Topic 8: Household Cleaning Agents & Homemade Soap

Validates:
1. Hierarchy Resolution (CBC -> Grade 7 -> Home Science -> Topic 8: Household Cleaning Agents & Homemade Soap, ID/Order: 8)
...
class TestCBCGrade7HomeScienceTopic8(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Household Cleaning Agents & Homemade Soap").first()

    def test_01_hierarchy_resolution(self):
        self.assertIsNotNone(self.curriculum, "CBC Curriculum missing!")
        self.assertIsNotNone(self.grade, "Grade 7 missing!")
        self.assertIsNotNone(self.subject, "Home Science Subject missing!")
        self.assertIsNotNone(self.topic, "Topic 8 (Household Cleaning Agents & Homemade Soap) missing!")
        self.assertEqual(self.topic.order, 8, f"Expected Topic 8 order to be 8, got {self.topic.order}")
        print(f"\n[QA PASS] Hierarchy correctly resolved to Grade 7 Topic 8: {self.topic.name} (ID: {self.topic.id}).")
