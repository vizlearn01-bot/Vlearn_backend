"""
VLearn CBC Grade 10 History — Topic 13: International Organisations (The Commonwealth)
...
Curriculum: CBC
Topic Order: 13
Topic Name: "Topic 3.2: International Organisations (The Commonwealth)"
"""
...
from curriculum.cbc_grade10_history_topic13_data import TOPIC_13_LESSONS
...
def ingest_grade10_history_topic13(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 13: International Organisations (The Commonwealth) (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.2: International Organisations (The Commonwealth)"
    topic_desc = "Why international organisations exist, the Commonwealth of Nations, London & Harare Declarations, five core pillars of cooperation, and the Citizen-to-Global Cooperation Chain."

    topic = Topic.objects.filter(subject=subject, order=13).first()
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 13: {topic.name} (ID: {topic.id})")

    if replace:
        print("[*] Replacing existing Topic 13 units, lessons, blocks, and assets...")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t13_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 13 INGESTION COMPLETE:")
...
if __name__ == "__main__":
    ingest_grade10_history_topic13(replace=True)
