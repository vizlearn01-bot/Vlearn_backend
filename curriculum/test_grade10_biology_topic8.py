...
Validates:
  1. Curriculum Hierarchy (CBC -> Grade 10 -> Biology -> Topic 8)
  2. Learning Units & Published Lessons 1-to-1 Parity (2 Comprehensive Lessons)
  3. Concept Card Distribution & Page Numbering (19 Total Cards)
  4. Block Integrity & Typing (Learning Goals, Explanations, Tables, Steps, Knowledge Checks)
  5. Visual Enrichment Assets (2 High-Quality SVGs, 2 Contextualized Photos, 2 YouTube Videos - 1 per lesson)
  6. Assessment Integrity (Valid Questions, Options, Answer Keys, Explanations)
  7. Content Leak Audit (Zero [VISUAL: ...] tags, zero citation brackets [52], zero corrupted LaTeX)

Usage:
  ./venv/bin/python curriculum/test_grade10_biology_topic8.py
"""
...
def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED QA: GRADE 10 BIOLOGY TOPIC 8 (ANIMAL NUTRITION & FEEDING ADAPTATIONS)")
    print("=" * 80)
    ...
    if not topic:
        errors.append("Topic 'Animal Nutrition and Feeding Adaptations' not found under Grade 10 Biology!")
    ...
    # 2. Units & Lessons Count (Expected: 2 Comprehensive Lessons)
    units = list(topic.learning_units.all().order_by("order"))
    lessons = list(topic.lessons.all().order_by("learning_unit__order"))

    if len(units) != 2:
        errors.append(f"Expected 2 LearningUnits, found {len(units)}")
    else:
        print(f"[PASS] Learning Units Count: {len(units)} (1-to-1 verified)")

    if len(lessons) != 2:
        errors.append(f"Expected 2 Published Lessons, found {len(lessons)}")
    else:
    ...
    print("-" * 80)
    print(f"Total Vector SVGs:      {total_svgs} blocks, {diagram_assets} LessonAsset records (Expected: 2)")
    print(f"Total Wikimedia Photos: {total_photos} blocks, {photo_assets} LessonAsset records (Expected: 2)")
    print(f"Total YouTube Videos:   {total_videos} blocks, {video_assets} LessonAsset records (Expected: 2)")
    print(f"Total LessonAssets:     {lesson_assets.count()} (Expected: 6)")

    if diagram_assets != 2:
        errors.append(f"Expected 2 diagram assets, found {diagram_assets}")
    if photo_assets != 2:
        errors.append(f"Expected 2 photo assets, found {photo_assets}")
    if video_assets != 2:
        errors.append(f"Expected 2 video assets, found {video_assets}")
    ...
    leak_patterns = [
        ...
        (r'\[(?:\d+|image_\d+)\]', "Raw Bracket Citation Leak [e.g. 52]"),
    ...
    print("\n[SUCCESS] ALL GRADE 10 BIOLOGY TOPIC 8 TESTS PASSED!")
    return True
