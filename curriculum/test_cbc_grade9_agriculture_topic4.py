"""
Automated Quality Assurance Test Suite for CBC Grade 9 Agriculture — Topic 4: Organic Gardening
Curriculum: CBC -> Grade 9 -> Agriculture -> Topic 4: Organic Gardening

Test Coverage:
  1. Hierarchy and Topic Configuration Integrity (Topic Order: 4)
  2. Exactly 8 LearningUnits and 8 Published Lessons
  ...
  6. Topic Video Review (YouTube ID: Ei5z_0Lxmic, verified status, attached LessonAsset)
  7. Zero Citation Bracket Leaks, Zero Developer Meta-Tags, Zero Raw LaTeX
  8. Formative (8) and Summative (10) MCQ Validation (18 Total Knowledge Checks)

Run:
  ./venv/bin/python curriculum/test_cbc_grade9_agriculture_topic4.py
"""
...
class TestCBCGrade9AgricultureTopic4(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic_name = "Organic Gardening"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.order, 4, "Topic 4 order must be 4")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(units.count(), 8, f"Expected exactly 8 LearningUnits, found {units.count()}")
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all 18 interactive knowledge_check blocks across the 8 lessons."""
        mcq_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        self.assertEqual(mcq_blocks.count(), 18, f"Expected 18 MCQs (8 formative + 10 summative), found {mcq_blocks.count()}")
