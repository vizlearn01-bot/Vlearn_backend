...
Curriculum: CBC
Topic Order: 14
Topic Name: "Topic 3.2A: Modern Slavery and Servitude"
"""
...
from curriculum.cbc_grade10_history_topic14_data import TOPIC_14_LESSONS
...
def ingest_grade10_history_topic14(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 14: Modern Slavery and Servitude (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.2A: Modern Slavery and Servitude"
    topic_desc = "Definitions of modern slavery and servitude, multi-causal drivers, debt bondage, the 4Ps Anti-Slavery Framework, ethical consumerism, and evaluating human rights evidence."
    ...
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 14: {topic.name} (ID: {topic.id})")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t14_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_history_topic14(replace=True)
