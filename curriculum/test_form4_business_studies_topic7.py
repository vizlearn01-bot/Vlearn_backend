"""
VLearn Form 4 Business Studies — Automated Test Suite: Topic 7 (International Trade)
"""
...
def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 BUSINESS STUDIES TOPIC 7 (INTERNATIONAL TRADE)")
    print("=" * 80)
    ...
    assert topic is not None, "Topic 7 (International Trade) not found!"
    assert topic.name == "International Trade", f"Unexpected topic name: {topic.name}"
    ...
    assert worked_examples >= 4, f"Expected at least 4 worked examples, got {worked_examples}"
    assert knowledge_checks >= 5, f"Expected at least 5 knowledge checks, got {knowledge_checks}"
    assert tables >= 4, f"Expected at least 4 comparison tables, got {tables}"

    print(f"[PASS] 11. Verified {worked_examples} worked examples, {knowledge_checks} knowledge checks, and {tables} comparison tables.")

    print("=" * 80)
    print("[ALL TESTS PASSED] Form 4 Business Studies Topic 7 is 100% verified and production-ready!")
    print("=" * 80)
