"""
Form 4 Agriculture — Topic 7 Automated Verification Suite
Topic: Agroforestry
Curriculum: 844 (ID: 4) | Grade: Form 4 (ID: 4) | Subject: Agriculture (ID: 19) | Topic ID: 101 (Order: 7)

Usage:
    ./venv/bin/python curriculum/test_form4_agriculture_topic7.py
"""
...
EXPECTED_TOPIC_ID = 101
EXPECTED_LESSON_COUNT = 4
EXPECTED_PAGE_COUNT = 40
EXPECTED_MIN_BLOCK_COUNT = 40
EXPECTED_ASSET_COUNT = 4

EXPECTED_UNITS = [
    "Definition, Forms, Importance, and Species Selection",
    "Nursery Types, Seed Collection, and Dormancy Breaking",
    "Nursery Management, Hardening Off, and Transplanting",
    "Tree Maintenance, Grafting Mechanics, and Sustainable Harvesting"
]


def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 AGRICULTURE TOPIC 7")
    print("TOPIC: AGROFORESTRY")
    print("=" * 80)
    ...
    assert topic.order == 7, f"Expected order 7, got {topic.order}"
    ...
    print("[PASS] 5. Zero citation brackets ([170], [182], [188]) found across all block contents.")
    ...
    assert mcq_blocks.count() == 4, f"Expected 4 MCQs, found {mcq_blocks.count()}"
    ...
    print("FORM 4 AGRICULTURE TOPIC 7 IS 100% VERIFIED & PRODUCTION-READY")
    print("=" * 80)
