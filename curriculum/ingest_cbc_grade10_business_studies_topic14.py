...
from curriculum.cbc_grade10_business_studies_topic14_data import TOPIC_14_LESSONS
...
def ingest_grade10_business_studies_topic14(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 14: Effects of Business Transactions (Grade 10 Business Studies)")
    print("=" * 80)
    ...
    subject, _ = Subject.objects.get_or_create(
        ...
        defaults={"description": "Senior Secondary Business Studies Curriculum (Grade 10 CBC)"}
    )
    ...
    topic, t_created = Topic.objects.get_or_create(
        ...
        defaults={
            ...
            "description": "Exploration of the fundamental accounting equation (Assets = Capital + Liabilities), structure of the Statement of Financial Position (Balance Sheet), classification of non-current and current items, transaction effect pathways, multi-period commercial simulations, and capital adjustments determining enterprise net worth."
        }
    )
    if not t_created:
        topic.name = "Effects of Business Transactions"
        topic.description = "Exploration of the fundamental accounting equation (Assets = Capital + Liabilities), structure of the Statement of Financial Position (Balance Sheet), classification of non-current and current items, transaction effect pathways, multi-period commercial simulations, and capital adjustments determining enterprise net worth."
        topic.save()
    print(f"[*] Resolved Topic 14: {topic.name} (ID: {topic.id})")

    if replace:
        print("[*] Replacing existing Topic 14 units, lessons, blocks, and assets...")
    ...
    with transaction.atomic():
        for item in TOPIC_14_LESSONS:
            ...
            lesson = Lesson.objects.create(
                ...
                version=1,
                immutable_metadata={
                    "author": "VLearn Senior Business Studies Curriculum Agent",
                    "grade": "Grade 10",
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_bs_t14_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 14 INGESTION COMPLETE:")
...
if __name__ == "__main__":
    ingest_grade10_business_studies_topic14(replace=True)
