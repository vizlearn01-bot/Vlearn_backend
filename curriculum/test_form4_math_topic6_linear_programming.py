#!/usr/bin/env python3
"""
VLearn Form 4 Mathematics — Topic 6: Linear Programming Test Suite
===================================================================
Validates:
  ...
  5. Content quality (zero developer leakages, zero raw math arrays, key LP concepts present)
...
class TestHierarchy(unittest.TestCase):
    """Verify the database hierarchy for Topic 6."""
    ...
    def test_topic_exists(self):
        topic = Topic.objects.filter(name__icontains="Linear Programming", subject__name="Mathematics").first()
        self.assertIsNotNone(topic, "Topic 'Linear Programming' not found")
        self.assertEqual(topic.order, 6, "Topic order should be 6")

    def test_exactly_4_learning_units(self):
        topic = Topic.objects.filter(name__icontains="Linear Programming").first()
...
class TestBlockCounts(unittest.TestCase):
    ...
    def setUp(self):
        self.topic = Topic.objects.filter(name__icontains="Linear Programming").first()
    ...
    def test_minimum_total_blocks(self):
        ...
        self.assertGreaterEqual(
            total_blocks, 40,
            f"Expected at least 40 blocks across Topic 6, found {total_blocks}"
        )
...
class TestContentQuality(unittest.TestCase):
    ...
    def test_key_lp_concepts_present(self):
        all_text = " ".join([json.dumps(b.content, default=str) for b in self.blocks])
        key_terms = [
            "constraint", "feasible region", "objective function", "corner",
            "search line", "unwanted", "inequality", "maximize", "minimize"
        ]
        for term in key_terms:
            self.assertIn(
                term.lower(), all_text.lower(),
                f"Key Linear Programming concept '{term}' not found in Topic 6 content"
            )
...
if __name__ == "__main__":
    print("=" * 80)
    print("Form 4 Mathematics Topic 6 (Linear Programming) — Validation Test Suite")
    print("=" * 80)
