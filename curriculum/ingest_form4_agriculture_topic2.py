"""
VLearn Form 4 Agriculture — Topic 2: Livestock Production VI (Cattle)
...
Usage:
  ./venv/bin/python curriculum/ingest_form4_agriculture_topic2.py [--replace]
"""
...
from curriculum.topic2_agriculture_data import ALL_LESSONS

def clean_text(raw_str):
    """Remove source citation brackets like [37], [38], [52] and normalize whitespace."""
...
def run_ingestion(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 AGRICULTURE TOPIC 2 INGESTION ENGINE")
    print("=" * 80)
    ...
    with transaction.atomic():
        ...
        topic, _ = Topic.objects.get_or_create(
            subject=subject,
            name="Livestock Production VI (Cattle)",
            defaults={
                "order": 2,
                "description": (
                    "Covers calf feeding, colostrum, early/late weaning, replacement stock, "
                    "calf housing, milk secretion & let-down, clean milking, dry-cow therapy, and marketing."
                )
            }
        )
        topic.order = 2
            ...
            for page_idx, page in enumerate(pages, 1):
                ...
                for comp_idx, block_info in enumerate(blocks, 1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"ag4_t2_l{unit_order}_p{page_num}_b{comp_idx}",
                        block_type=b_type,
        ...
        print("\n" + "=" * 80)
        print(f"FORM 4 AGRICULTURE TOPIC 2 INGESTION COMPLETE")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Agriculture Topic 2")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 2 data")
