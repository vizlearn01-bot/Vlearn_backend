...
CBC Grade 6 Home Science — Topic 1: Adolescence

Validates:
1. Hierarchy Resolution (CBC -> Grade 6 -> Home Science -> Topic 1: Adolescence, Order: 1)
...
class TestCBCGrade6HomeScienceTopic1(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Adolescence").first()
    ...
    def test_01_hierarchy_resolution(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 1 (Adolescence) missing!")
        self.assertEqual(self.topic.order, 1, f"Expected Topic 1 order to be 1, got {self.topic.order}")
        print(f"\n[QA PASS] Hierarchy correctly resolved to Grade 6 Topic 1: {self.topic.name} (ID: {self.topic.id}).")
