...
Tests:
  ...
  2. test_02_units_and_published_lessons_count: Verifies exactly 8 Units and 8 published Lessons.
  3. test_03_card_and_block_structure: Verifies card pagination and block distribution.
  4. test_04_card_1_visual_hooks: Verifies all 8 lessons have Page 1 Wikimedia image hooks and attached assets.
  5. test_05_custom_vector_svgs_sanitization: Verifies 8 sanitized responsive vector SVGs.
  6. test_06_multi_video_integration: Verifies YouTube video lessons across Units 4, 7, and 8.
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade8_agriculture_topic5.py
"""
...
class TestCBCGrade8AgricultureTopic5(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Crop Pest and Disease Control").first()
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 'Crop Pest and Disease Control' must exist")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 8, f"Expected exactly 8 LearningUnits, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 8, f"Expected exactly 8 Lessons, found {len(self.lessons)}")
    ...
    def test_05_custom_vector_svgs_sanitization(self):
        """Verify all 8 lessons have sanitized responsive vector SVGs attached."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_07_zero_citation_and_meta_leaks(self):
        """Audit all blocks for citation brackets [1], [112], developer meta-terms, and raw unrendered LaTeX."""
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all interactive knowledge_check blocks across the 8 lessons."""
        kc_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        # 1 formative per lesson (1-7) + 1 formative + 10 summative on lesson 8 = 18 MCQs
        self.assertGreaterEqual(kc_blocks.count(), 17, f"Expected at least 17 MCQs, found {kc_blocks.count()}")
