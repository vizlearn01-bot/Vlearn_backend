...
class TestCBCGrade10BusinessStudiesTopic9(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 9 'Production' not found in database!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Production")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 9, f"Expected 9 units in Topic 9, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 9, f"Expected 9 lessons in Topic 9, found {len(self.lessons)}")
    ...
    def test_03_card_and_block_structure(self):
        """Verify every lesson has valid page counts (8 pages) and non-empty blocks."""
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 9 lessons have Page 1 photographic visual hooks with Wikimedia URLs."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 9 lessons contain responsive vector SVGs attached as LessonAssets."""
        for u_order in range(1, 10):
    ...
    def test_06_youtube_video_assets(self):
        """Verify 100% YouTube video coverage across all 9 lessons."""
        video_count = 0
        for lesson in self.lessons:
            ...
            if yt_asset or yt_block:
                video_count += 1
        self.assertEqual(video_count, len(self.lessons), f"Expected 100% video coverage in Topic 9, found {video_count}/{len(self.lessons)}")
