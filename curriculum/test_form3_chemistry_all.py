"""
Master Comprehensive Test Suite for Form 3 Chemistry (All 6 Topics)
"""
...
def test_all_form3_chemistry():
    print("=" * 80)
    print("RUNNING MASTER COMPREHENSIVE AUDIT & VALIDATION: FORM 3 CHEMISTRY")
    print("=" * 80)
    ...
    for topic in topics:
        ...
        for lesson in lessons:
            ...
            # Check that every lesson has at least 1 image
            ...
            for img in images:
                assert "States_of_matter" not in str(img.url) or "Gas Laws" in topic.name, f"Lesson {lesson.id} has incorrect placeholder image"
                assert "Diffusion_of_ammonia" not in str(img.url) or "Graham" in lesson.title, f"Lesson {lesson.id} has incorrect placeholder image"
    ...
    print("\n" + "=" * 80)
    print("MASTER FORM 3 VALIDATION SUMMARY:")
    ...
    print(f"  • Generic Duplicate Placeholders: 0")
    print(f"  • Trailing '\\ \\text{{}}' LaTeX Artefacts: 0")
    print(f"  • Student-Facing Meta Generation Labels: 0")
    print("=" * 80)
    print("🎉 ALL FORM 3 CHEMISTRY LESSONS PASSED RIGOROUS MASTER AUDIT!")

if __name__ == "__main__":
    test_all_form3_chemistry()
