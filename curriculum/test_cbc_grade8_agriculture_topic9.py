...
Tests:
  ...
  2. test_02_units_and_published_lessons_count: Verifies exactly 5 Units and 5 published Lessons.
  3. test_03_card_and_block_structure: Verifies card pagination and block distribution.
  4. test_04_card_1_visual_hooks: Verifies all 5 lessons have Page 1 Wikimedia image hooks and attached assets.
  5. test_05_custom_vector_svgs_sanitization: Verifies 5 sanitized responsive vector SVGs.
  6. test_06_multi_video_integration: Verifies YouTube video lessons across Units 2, 4, and 5.
  7. test_07_zero_citation_and_meta_leaks: Audits zero citation leaks, zero meta-tags, and zero raw LaTeX in student text.
  8. test_08_formative_and_summative_mcqs_validation: Verifies all interactive knowledge check MCQs.

Usage:
  ./venv/bin/python curriculum/test_cbc_grade8_agriculture_topic9.py
"""
...
class TestCBCGrade8AgricultureTopic9(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Cooking Balanced Meals for Special Occasions").first()
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 'Cooking Balanced Meals for Special Occasions' must exist")
        self.assertEqual(self.topic.order, 9, "Topic order must be 9")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 5, f"Expected exactly 5 LearningUnits, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 5, f"Expected exactly 5 Lessons, found {len(self.lessons)}")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 5 lessons have Page 1 photographic visual hooks with HTTP 200 URLs & attached LessonAssets."""
    ...
    def test_05_custom_vector_svgs_sanitization(self):
        """Verify all 5 lessons have sanitized responsive vector SVGs attached."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_07_zero_citation_and_meta_leaks(self):
        """Audit all blocks for citation brackets [1], [Topic 4], developer meta-terms, and raw unrendered LaTeX."""
        citation_pattern = re.compile(r'\[(?:\d+|image_\d+|S\d+.*?|[\d,\s]{2,}|Topic \d+|Week \d+: Lesson \d+)\]')
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all interactive knowledge_check blocks across the 5 lessons."""
        kc_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        # 1 formative per lesson (1-4) + 1 formative + 10 summative on lesson 5 = 15 MCQs
        self.assertGreaterEqual(kc_blocks.count(), 14, f"Expected at least 14 MCQs, found {kc_blocks.count()}")
