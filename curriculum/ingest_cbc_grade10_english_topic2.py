...
from curriculum.cbc_grade10_english_topic2_data import TOPIC_2_LESSONS_ALL
...
def ingest_grade10_english_topic2(replace=True):
    print("=" * 80)
    print("INGESTING CBC GRADE 10 ENGLISH — TOPIC 2: READING (LESSONS 1 TO 10)")
    print("=" * 80)
    ...
    # 3. Resolve or Create Topic 2 'Reading' (Order 2)
    topic, t_created = Topic.objects.get_or_create(
        subject=subject,
        order=2,
        defaults={
            "name": "Reading",
            "description": "Mastering reading fluency, extensive reading stamina, speed-reading strategies (skimming/scanning), text organization, inference, vocabulary context, SQ4R study skills, critical evaluation, reference materials, and multi-source synthesis."
        }
    )
    if not t_created:
        topic.name = "Reading"
        topic.description = "Mastering reading fluency, extensive reading stamina, speed-reading strategies (skimming/scanning), text organization, inference, vocabulary context, SQ4R study skills, critical evaluation, reference materials, and multi-source synthesis."
        topic.save()
        print(f"[*] Updated Topic 2: {topic.name} (ID: {topic.id})")
    else:
    ...
    with transaction.atomic():
        for item in TOPIC_2_LESSONS_ALL:
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_eng_t2_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_english_topic2(replace=True)
