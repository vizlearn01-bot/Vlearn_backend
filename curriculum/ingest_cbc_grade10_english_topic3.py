...
from curriculum.cbc_grade10_english_topic3_data import TOPIC_3_LESSONS
...
def ingest_grade10_english_topic3(replace=True):
    print("=" * 80)
    print("INGESTING CBC GRADE 10 ENGLISH — TOPIC 3: GRAMMAR IN USE (LESSONS 1 TO 10)")
    print("=" * 80)
    ...
    topic, t_created = Topic.objects.get_or_create(
        ...
        defaults={
            "name": "Grammar in Use",
            "description": "Mastering grammatical systems, word classes, phrase architecture, clause structures, sentence fluency, and active/passive voice in use."
        }
    )
    if not t_created:
        topic.name = "Grammar in Use"
        topic.description = "Mastering grammatical systems, word classes, phrase architecture, clause structures, sentence fluency, and active/passive voice in use."
        topic.save()
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_eng_t3_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_english_topic3(replace=True)
