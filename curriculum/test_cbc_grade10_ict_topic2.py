"""
VLearn CBC Grade 10 ICT — Topic 2: Application Areas of ICT
...
class TestCBCGrade10ICTTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 2 'Application Areas of ICT' not found under ICT!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Application Areas of ICT")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 4 lessons have Page 1 photographic visual hooks with Wikimedia URLs."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 4 lessons contain sanitized, responsive vector SVGs attached as LessonAssets."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_06_youtube_video_assets(self):
        """Verify YouTube video coverage across all 4 lessons (100%)."""
        video_count = 0
        for lesson in self.lessons:
            ...
            self.assertIsNotNone(video_block, f"Lesson {lesson.learning_unit.order} missing suggested_video block!")
            content = video_block.content or {}
            yt_id = content.get("youtube_id")
            self.assertTrue(yt_id, f"Lesson {lesson.learning_unit.order} video missing youtube_id")
        ...
        print(f"[*] Topic 2 Video Coverage: {video_count}/{len(self.lessons)} ({coverage:.1f}%)")
        self.assertEqual(coverage, 100.0, f"Topic 2 video coverage below 100%: {coverage}%")

    def test_07_content_sanitization(self):
        """Verify zero bracket citations ([20], [44]), zero developer prompt leakage, and clean markdown."""
        for lesson in self.lessons:
            ...
            for b in blocks:
                ...
                for leak in ["Prompt:", "JSON Payload", "block_type:", "component_order:", "[VISUAL:", "[MISCONCEPTION]"]:
    ...
    def test_08_formative_mcqs(self):
        ...
        for lesson in self.lessons:
            ...
            self.assertGreaterEqual(len(mcq_blocks), 2, f"Lesson {lesson.id} must have >= 2 MCQs, got {len(mcq_blocks)}")
        ...
        self.assertGreaterEqual(mcq_count, 8, f"Expected >= 8 MCQs in Topic 2, found {mcq_count}")
