#!/usr/bin/env python3
"""
VLearn Form 4 Mathematics — Topic 8: Differentiation Test Suite
================================================================
Validates:
  ...
  5. Content quality (zero developer leakages, zero raw math arrays, key Differentiation concepts present)
...
from curriculum.ingest_form4_math_topic8_differentiation import ingest_topic8_differentiation


class TestHierarchy(unittest.TestCase):
    """Verify the database hierarchy for Topic 8."""
    ...
    def test_topic_exists(self):
        topic = Topic.objects.filter(name__icontains="Differentiation", subject__name="Mathematics").first()
        self.assertIsNotNone(topic, "Topic 'Differentiation' not found")
        self.assertEqual(topic.order, 8, "Topic order should be 8")

    def test_exactly_4_learning_units(self):
        topic = Topic.objects.filter(name__icontains="Differentiation").first()
...
class TestBlockCounts(unittest.TestCase):
    ...
    def setUp(self):
        self.topic = Topic.objects.filter(name__icontains="Differentiation").first()
    ...
    def test_minimum_total_blocks(self):
        ...
        self.assertGreaterEqual(
            total_blocks, 40,
            f"Expected at least 40 blocks across Topic 8, found {total_blocks}"
        )
...
class TestContentQuality(unittest.TestCase):
    ...
    def test_key_differentiation_concepts_present(self):
        all_text = " ".join([json.dumps(b.content, default=str) for b in self.blocks])
        key_terms = [
            "derivative", "tangent", "normal", "stationary", "power rule",
            "velocity", "acceleration", "optimization", "first principles"
        ]
        for term in key_terms:
            self.assertIn(
                term.lower(), all_text.lower(),
                f"Key Differentiation concept '{term}' not found in Topic 8 content"
            )
...
if __name__ == "__main__":
    print("=" * 80)
    print("Form 4 Mathematics Topic 8 (Differentiation) — Validation Test Suite")
    print("=" * 80)
