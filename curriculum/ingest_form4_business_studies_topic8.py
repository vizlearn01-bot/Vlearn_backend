"""
VLearn Form 4 Business Studies — Topic 8 Ingestion Engine (Economic Integration)
...
from curriculum.topic8_data import TOPIC8_UNITS
...
@transaction.atomic
def ingest_topic8(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 8 INGESTION ENGINE (ECONOMIC INTEGRATION)")
    print("=" * 80)
    ...
    # 2. Topic 8: Economic Integration
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=8,
        defaults={
            "name": "Economic Integration",
            "description": "Meaning, forms and stages of economic integration (Free Trade Area, Customs Union, Common Market, Economic Union), Common External Tariff (CET), factor mobility, benefits and demerits of trade blocs, Trade Creation vs Trade Diversion, and East African Community (EAC) applications."
        }
    )
    if topic.name != "Economic Integration":
        topic.name = "Economic Integration"
        topic.description = "Meaning, forms and stages of economic integration (Free Trade Area, Customs Union, Common Market, Economic Union), Common External Tariff (CET), factor mobility, benefits and demerits of trade blocs, Trade Creation vs Trade Diversion, and East African Community (EAC) applications."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 8...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC8_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 8 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L8U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 8 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 8")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 8 content")
    args = parser.parse_args()

    ingest_topic8(replace=args.replace)
