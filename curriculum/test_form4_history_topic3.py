"""
VLearn Form 4 History — Automated Test Suite for Topic 3: Co-operation in Africa

Verifies:
  1. Target Subject, Grade, Curriculum mapping (History, Form 4, 844)
  2. Topic 3 existence and ordering (Order: 3)
  3. Exact 7 Learning Units and 7 Lessons
  4. Exact 113 Pages across the 7 Lessons (15, 15, 16, 16, 18, 15, 18)
  ...
  7. Zero bracket citations ([64], [223], [1]) across all blocks
  8. Zero internal developer/generation metadata leaks
  9. 100% LessonAsset associations (Images + Videos)

Usage:
  ./venv/bin/python curriculum/test_form4_history_topic3.py
"""
...
def test_form4_history_topic3():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 HISTORY TOPIC 3 (CO-OPERATION IN AFRICA)")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="Co-operation in Africa").first()
    assert topic, "Topic 'Co-operation in Africa' not found!"
    ...
    print("[PASS] 4.1 Every single lesson in Topic 3 has an establishing real-world visual on Page 1.")
    ...
    expected_pages = {
        1: 15, # Pan-Africanism: Origins, Early Pioneers, Diaspora Era
        2: 15, # The Manchester Congress (1945) & Return to African Soil
        3: 16, # The Organisation of African Unity: Structure, Successes, Failures
        4: 16, # The African Union: Evolution, Governance, Security Architecture
        5: 18, # The East African Community: Evolution, 1977 Collapse, 2001 Rebirth
        6: 15, # ECOWAS: Regional Integration, ECOMOG Peacekeeping, Dynamics
        7: 18  # COMESA and Continental Economic Integration in Africa
    }
    ...
    assert total_pages == 113, f"Total pages is {total_pages}, expected 113"
    print(f"[PASS] 5. Total Pages across all 7 Lessons: {total_pages} (Matches target: 113)")

    # 6. Check Wikimedia Photographic Assets
    image_blocks = [b for b in all_blocks if b.block_type == "suggested_image"]
    assert len(image_blocks) >= 7, f"Expected at least 7 suggested_image blocks, found {len(image_blocks)}"
    ...
    assert len(video_blocks) >= 7, f"Expected at least 7 suggested_video blocks, found {len(video_blocks)}"
    ...
    assert len(assets) >= 14, f"Expected at least 14 LessonAsset records, found {len(assets)}"
    for a in assets:
    ...
    # 9. Check for Bracket Citation Leakage ([64], [223], [1])
    ...
    print("=" * 80)
    print(f"[ALL TESTS PASSED] Form 4 History Topic 3 is fully verified, robust, and production-ready!")
