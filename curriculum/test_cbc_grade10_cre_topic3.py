...
Verifies:
  1. Hierarchy & Database Integrity (Curriculum CBC -> Grade 10 -> CRE -> Topic 1.3)
  2. 6 Discrete Learning Units & 6 Published Lessons (status='published', version=1)
  3. Dynamic 6-Card Progressive Structure (>= 6 pages per lesson, non-empty blocks)
  4. Mandatory First-Card Photographic Visual Hooks (6 Verified Wikimedia Commons URLs & attached LessonAssets)
  5. 6 Custom Sanitized Responsive Vector SVGs (viewBox="0 0 800 450", #0f172a theme, zero raw XML headers)
  6. 6 Curated Educational YouTube Videos (embed IDs & attached LessonAssets)
...
class TestCBCGrade10CRETopic1_3(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 1.3 (Order: 3) not found under Grade 10 CRE!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertIn("Redemption after the Fall of Man", self.topic.name)
    ...
    def test_03_card_and_block_structure(self):
        ...
        for lesson in self.lessons:
            ...
            self.assertGreaterEqual(len(blocks), 12, f"Lesson {lesson.id} has too few blocks ({len(blocks)})")
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 6 lessons have custom sanitized responsive vector SVGs with viewBox and attached LessonAssets."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_06_video_assets(self):
        """Verify all 6 lessons have curated educational YouTube video blocks and attached LessonAssets."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_08_mcq_integrity(self):
        ...
        self.assertEqual(len(mcq_blocks), 6, f"Expected 6 MCQ blocks, found {len(mcq_blocks)}")
