#!/usr/bin/env python3
"""
VLearn Form 4 Mathematics — Topic 4: Trigonometry III Test Suite
===================================================================
Validates:
  ...
  5. Content quality (zero developer leakages, zero raw math arrays, key trigonometry concepts present)
...
from curriculum.ingest_form4_math_topic4_trigonometry import ingest_topic4_trigonometry


class TestHierarchy(unittest.TestCase):
    """Verify the database hierarchy for Topic 4."""
    ...
    def test_topic_exists(self):
        topic = Topic.objects.filter(name__icontains="Trigonometry III", subject__name="Mathematics").first()
        self.assertIsNotNone(topic, "Topic 'Trigonometry III' not found")
        self.assertEqual(topic.order, 4, "Topic order should be 4")

    def test_exactly_4_learning_units(self):
        topic = Topic.objects.filter(name__icontains="Trigonometry III").first()
...
class TestBlockCounts(unittest.TestCase):
    ...
    def setUp(self):
        self.topic = Topic.objects.filter(name__icontains="Trigonometry III").first()
    ...
    def test_minimum_total_blocks(self):
        ...
        self.assertGreaterEqual(
            total_blocks, 40,
            f"Expected at least 40 blocks across Topic 4, found {total_blocks}"
        )
...
class TestContentQuality(unittest.TestCase):
    ...
    def test_key_trigonometry_concepts_present(self):
        all_text = " ".join([json.dumps(b.content, default=str) for b in self.blocks])
        key_terms = [
            "sin^2", "cos^2", "amplitude", "period", "phase shift",
            "CAST", "quadrant", "tan", "wave"
        ]
        for term in key_terms:
            self.assertIn(
                term.lower(), all_text.lower(),
                f"Key Trigonometry concept '{term}' not found in Topic 4 content"
            )
...
if __name__ == "__main__":
    print("=" * 80)
    print("Form 4 Mathematics Topic 4 (Trigonometry III) — Validation Test Suite")
    print("=" * 80)
