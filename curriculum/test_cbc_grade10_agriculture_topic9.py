...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 9: Animal Handling and Safety)
  2. 9 Learning Units & 9 Published Lessons Validation
  3. Dynamic Card & Block Structure Integrity (all lessons >= 4 pages, non-empty content)
  4. Mandatory First-Card Visual Hooks (9 Tested Wikimedia URLs & attached LessonAssets)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic9.py
"""
...
class TestCBCGrade10AgricultureTopic9(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Animal Handling and Safety").first()
        assert cls.topic, "Topic 9 'Animal Handling and Safety' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Animal Handling and Safety")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 9, f"Expected 9 units, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 9, f"Expected 9 lessons, found {len(self.lessons)}")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 3, 4, 5, 6, 7, 9]
    ...
    def test_08_formative_and_summative_mcqs(self):
        """Verify formative MCQs in Lessons 1-8 and Summative MCQs in Lesson 9 have 4 options, key, and explanation."""
        total_mcqs = 0
        for lesson in self.lessons:
        ...
        # Total MCQs = 9 (one per lesson 1-8 plus formative) + 8 (summative in lesson 9) = 17 MCQs
        self.assertGreaterEqual(total_mcqs, 16, f"Expected at least 16 MCQs, found {total_mcqs}")
