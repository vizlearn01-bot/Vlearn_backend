...
from curriculum.enrich_form4_math_topic8_calculus import enrich_topic8_calculus

class Form4MathTopic8EnrichmentTestCase(TestCase):
    def setUp(self):
        ingest_topic8_differentiation()
        enrich_topic8_calculus()

    def test_topic8_enrichment_blocks_and_assets(self):
        topic = Topic.objects.filter(subject__name='Mathematics', order=8).first()
        self.assertIsNotNone(topic, "Topic 8 Mathematics should exist")

        lessons = list(Lesson.objects.filter(topic=topic).order_by('id'))
        self.assertEqual(len(lessons), 4, "Topic 8 should have 4 lessons")

        # Lesson 1: Rates of Change & First Principles
        l1 = lessons[0]
        ...
        # Lesson 2: Polynomial Differentiation Mechanics
        l2 = lessons[1]
        ...
        # Lesson 3: Tangents, Normals, and Turning Points
        l3 = lessons[2]
        ...
        # Lesson 4: Kinematics Applications & Optimization
        l4 = lessons[3]
        ...
        print("ALL TOPIC 8 DIFFERENTIATION ENRICHMENT ASSERTIONS PASSED PERFECTLY!")
