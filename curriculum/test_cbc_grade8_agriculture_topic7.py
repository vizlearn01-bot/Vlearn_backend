...
Tests:
  ...
  2. test_02_units_and_published_lessons_count: Verifies exactly 6 Units and 6 published Lessons.
  3. test_03_card_and_block_structure: Verifies card pagination and block distribution.
  4. test_04_card_1_visual_hooks: Verifies all 6 lessons have Page 1 Wikimedia image hooks and attached assets.
  5. test_05_custom_vector_svgs_sanitization: Verifies 6 sanitized responsive vector SVGs.
  6. test_06_multi_video_integration: Verifies YouTube video lessons across Units 3, 5, and 6.
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade8_agriculture_topic7.py
"""
...
class TestCBCGrade8AgricultureTopic7(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Kitchen Hygiene Practices").first()
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 'Kitchen Hygiene Practices' must exist")
        self.assertEqual(self.topic.order, 7, "Topic order must be 7")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 6, f"Expected exactly 6 LearningUnits, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 6, f"Expected exactly 6 Lessons, found {len(self.lessons)}")
    ...
    def test_05_custom_vector_svgs_sanitization(self):
        """Verify all 6 lessons have sanitized responsive vector SVGs attached."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_07_zero_citation_and_meta_leaks(self):
        """Audit all blocks for citation brackets [543], [547], developer meta-terms, and raw unrendered LaTeX."""
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all interactive knowledge_check blocks across the 6 lessons."""
        kc_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        # 1 formative per lesson (1-5) + 1 formative + 10 summative on lesson 6 = 16 MCQs
        self.assertGreaterEqual(kc_blocks.count(), 15, f"Expected at least 15 MCQs, found {kc_blocks.count()}")
