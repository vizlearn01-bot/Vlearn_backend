...
from curriculum.enrich_form4_math_topic4_trigonometry import enrich_topic4_trigonometry

class Form4MathTopic4EnrichmentTestCase(TestCase):
    def setUp(self):
        ingest_topic4_trigonometry()
        enrich_topic4_trigonometry()

    def test_topic4_enrichment_blocks_and_assets(self):
        topic = Topic.objects.filter(subject__name='Mathematics', order=4).first()
        self.assertIsNotNone(topic, "Topic 4 Mathematics should exist")

        lessons = list(Lesson.objects.filter(topic=topic).order_by('id'))
        self.assertEqual(len(lessons), 4, "Topic 4 should have 4 lessons")

        # Lesson 1: Pythagorean Identity & Substitution
        l1 = lessons[0]
        ...
        # Lesson 2: Wave Functions: Amplitude and Period
        l2 = lessons[1]
        ...
        # Lesson 3: Wave Transformations and Phase Shifts
        l3 = lessons[2]
        ...
        p3_b4 = LessonBlock.objects.filter(lesson=l4, page_number=3, block_type='suggested_diagram').first()
        self.assertIsNotNone(p3_b4, "L4 Page 3 diagram block should exist")
        ...
        print("ALL TOPIC 4 TRIGONOMETRY III ENRICHMENT ASSERTIONS PASSED PERFECTLY!")
