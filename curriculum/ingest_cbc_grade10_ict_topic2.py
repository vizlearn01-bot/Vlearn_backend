...
from curriculum.cbc_grade10_ict_topic2_data import TOPIC_2_LESSONS

def clean_text(text: str) -> str:
    """Removes bracket citations, developer tags, and normalizes unicode bullets into standard markdown."""
    if not text:
        return ""
    text = re.sub(r'\[(?:\d+|image_\d+|S\d+.*?|[\d,\s]{2,})\]', '', text)
    text = re.sub(r'\[VISUAL:\s*[^\]]+\]', '', text, flags=re.IGNORECASE)
    text = re.sub(r'\[MISCONCEPTION\]', '', text, flags=re.IGNORECASE)
...
def ingest_grade10_ict_topic2(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 2: Application Areas of ICT (Grade 10 ICT)")
    print("=" * 80)
    ...
    topic, t_created = Topic.objects.get_or_create(
        ...
        defaults={
            ...
            "description": "Exploration of how ICT transforms education, communication, business, commerce, healthcare, governance, agriculture, entertainment, and transportation."
        }
    )
    if not t_created:
        topic.name = "Application Areas of ICT"
        topic.description = "Exploration of how ICT transforms education, communication, business, commerce, healthcare, governance, agriculture, entertainment, and transportation."
        topic.save()
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_ict_t2_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_ict_topic2(replace=True)
