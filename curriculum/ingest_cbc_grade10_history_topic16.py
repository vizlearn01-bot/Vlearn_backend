...
Curriculum: CBC
Topic Order: 16
Topic Name: "Topic 3.2E: Fourth-Generation Technologies and Historical Information"
"""
...
from curriculum.cbc_grade10_history_topic16_data import TOPIC_16_LESSONS
...
def ingest_grade10_history_topic16(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 16: Fourth-Generation Technologies and Historical Information (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.2E: Fourth-Generation Technologies and Historical Information"
    topic_desc = "Core technologies of 4IR (AI, IoT, Cloud, BioTech), sustainability opportunities and systemic risks, transforming historical research with VR/AR, and Digital Historical Literacy."

    topic = Topic.objects.filter(subject=subject, order=16).first()
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 16: {topic.name} (ID: {topic.id})")

    if replace:
        print("[*] Replacing existing Topic 16 units, lessons, blocks, and assets...")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t16_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 16 INGESTION COMPLETE:")
