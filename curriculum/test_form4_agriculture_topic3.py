"""
VLearn Form 4 Agriculture — Automated Test Suite for Topic 3: Farm Power and Machinery

Verifies:
  1. Hierarchy resolution: Curriculum 844 -> Form 4 -> Agriculture (Subject ID: 19)
  2. Topic 3 existence ("Farm Power and Machinery")
  3. Exact 5 Learning Units and 5 Lessons
  4. Exact 50 Pages across all lessons
  5. 100% presence of verified Wikimedia URLs and attributions
  6. 100% presence of valid SVG diagrams
  7. Zero citation brackets ([56], [61], [107]) across all blocks
...
Usage:
  ./venv/bin/python curriculum/test_form4_agriculture_topic3.py
"""
...
def run_test_suite():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 AGRICULTURE TOPIC 3")
    print("TOPIC: FARM POWER AND MACHINERY")
    print("=" * 80)
    ...
    topic = Topic.objects.filter(subject=subject, name="Farm Power and Machinery").first()
    assert topic, "Topic 'Farm Power and Machinery' not found!"
    ...
    print(f"[PASS] 3. Verified 5 Learning Units and 5 Published Lessons:")
    for u in units:
    ...
    assert total_pages == 50, f"Expected 50 Total Pages, found {total_pages}"
    print(f"[PASS] 4. Page & Block Structure verified: {total_pages} Pages, {total_blocks} Blocks across 5 Lessons.")
    ...
    print("[PASS] 5. Zero citation brackets ([56], [61], [107]) found across all block contents.")
    ...
    assert mcq_blocks.count() >= 5, f"Expected at least 5 MCQ blocks, found {mcq_blocks.count()}"
    for b in mcq_blocks:
        content = b.content
    ...
    print("FORM 4 AGRICULTURE TOPIC 3 IS 100% VERIFIED & PRODUCTION-READY")
    print("=" * 80)
