...
from curriculum.cbc_grade9_english_topic2_data import TOPIC_2_LESSONS_ALL
...
def ingest_grade9_english_topic2(replace=True):
    print("=" * 80)
    print("INGESTING CBC GRADE 9 ENGLISH — TOPIC 2: READING AND LITERATURE (LESSONS 1 TO 7)")
    print("=" * 80)
    ...
    # 3. Resolve or Create Topic 2 'Reading and Literature' (Order 2)
    topic_description = (
        "Mastering reading fluency, speed strategies (skimming/scanning), sensory visualization, "
        "SQ4R note-making, oral literature short forms (riddles, proverbs, tongue twisters), "
        "poetry analysis (rhyme schemes, personification, persona), and dramatic literature "
        "(script formatting, Freytag plot arc, characterisation, monologue, flashback, themes, and community lessons)."
    )
    ...
    if not t_created:
        topic.name = "Reading and Literature"
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g9_eng_t2_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade9_english_topic2(replace=True)
