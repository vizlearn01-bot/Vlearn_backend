...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 7: General Crop Harvesting)
  ...
  6. 1 Verified Educational YouTube Video Asset (Lesson 2)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic7.py
"""
...
class TestCBCGrade10AgricultureTopic7(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="General Crop Harvesting").first()
        assert cls.topic, "Topic 7 'General Crop Harvesting' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "General Crop Harvesting")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 3, 4, 6, 8, 9, 10]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video asset in Lesson 2."""
        lesson = next((l for l in self.lessons if l.learning_unit.order == 2), None)
        video_block = LessonBlock.objects.filter(
            lesson=lesson,
            block_type="suggested_video"
        ).first()
        self.assertIsNotNone(video_block, "Lesson 2 missing suggested_video block!")

        content = video_block.content or {}
        v_url = content.get("url")
        self.assertTrue(v_url and "youtube.com" in v_url, f"Lesson 2 video URL invalid: {v_url}")

        asset = video_block.assets.filter(asset_type="youtube").first()
        self.assertIsNotNone(asset, "Lesson 2 video missing attached LessonAsset!")
        self.assertEqual(asset.status, "attached")
    ...
    def test_08_formative_and_summative_mcqs(self):
        ...
        # Total MCQs = 10 (one per lesson 1-9 plus formative) + 8 (summative in lesson 10) = 18 MCQs
