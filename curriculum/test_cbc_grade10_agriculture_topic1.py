...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 1: Agricultural Land)
  2. 10 Learning Units & 10 Published Lessons Validation
  3. Dynamic Card & Block Structure Integrity (all lessons >= 6 pages, non-empty content)
  4. Mandatory First-Card Visual Hooks (10 Tested HTTP 200 OK URLs & attached LessonAssets)
  ...
  8. Formative & Summative Scenario-Based MCQs Validation (18 MCQs with 4 Options, Valid Answer Key & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic1.py
"""
...
class TestCBCGrade10AgricultureTopic1(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Agricultural Land").first()
        assert cls.topic, "Topic 1 'Agricultural Land' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Agricultural Land")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 3, 4, 6, 7, 8, 9]
    ...
    def test_07_content_sanitization(self):
        ...
        raw_latex_leak_pattern = re.compile(r'(?<!\$)\\[a-zA-Z]+(?:\{.*?\})?(?!\$)')
        ...
        for lesson in self.lessons:
            ...
            for b in blocks:
                ...
                # Recursively check string contents
                def check_strings(val, path=""):
                    if isinstance(val, str):
                        # Exclude SVG payloads from raw LaTeX check
                        if "<svg" in val:
                            return
                        self.assertFalse(
