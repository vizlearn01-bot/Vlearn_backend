...
Tests:
- Curriculum hierarchy integrity (CBC -> Grade 10 -> ICT -> Topic 6 "Desktop Publishing")
- Exactly 6 LearningUnits and 6 published Lessons
- Exactly 5 pages per lesson following the 5-page pedagogical card architecture
- Page 1 visual hook with Wikimedia image and learning goals
- Page 2 definition card and concept explanation
- Page 3 custom vector SVG diagram with attached LessonAsset and step-by-step procedures
- Page 4 real-world example, common misconception, and troubleshooting mistake
- Page 5 formative MCQs (4 options, valid answer key A/B/C/D, explanation), YouTube video asset, and summary
- Zero bracket citations and zero prompt leaks
"""
...
class TestCBCGrade10ICTTopic6(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 6 'Desktop Publishing' not found under ICT!"
    ...
    def test_01_hierarchy_integrity(self):
        """Verify the full curriculum hierarchy and Topic 6 configuration."""
        ...
        self.assertEqual(self.topic.name, "Desktop Publishing")
        self.assertTrue(self.topic.description, "Topic 6 missing description")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 6 lessons have Page 1 photographic visual hooks with Wikimedia URLs and learning goals."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 6 lessons contain sanitized, responsive vector SVGs attached as LessonAssets on Page 3."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_06_youtube_video_assets(self):
        ...
        print(f"[*] Topic 6 Video Coverage: {video_count}/{len(self.lessons)} ({coverage:.1f}%)")
    ...
    def test_08_formative_mcqs(self):
        ...
        self.assertGreaterEqual(mcq_count, 12, f"Expected >= 12 MCQs across Topic 6, found {mcq_count}")

    def test_09_pedagogical_page_completeness(self):
        """Verify the full 5-page pedagogical card architecture across all 6 lessons."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
