...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 6: Crop Protection)
  ...
  5. 10 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. 2 Verified Educational YouTube Video Assets (Lesson 3 & Lesson 12)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic6.py
"""
...
class TestCBCGrade10AgricultureTopic6(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Crop Protection").first()
        assert cls.topic, "Topic 6 'Crop Protection' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Crop Protection")
    ...
    def test_05_custom_vector_svgs(self):
        """Verify 10 custom responsive SVGs have viewBox, are sanitized, and have attached LessonAssets."""
        expected_svg_units = [1, 2, 3, 4, 5, 7, 9, 10, 11, 14]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video assets in Lessons 3 and 12."""
        for u_order in [3, 12]:
            lesson = next((l for l in self.lessons if l.learning_unit.order == u_order), None)
