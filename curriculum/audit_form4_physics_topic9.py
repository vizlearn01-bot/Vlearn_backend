"""
Form 4 Physics — Topic 9 Ingestion Audit Tool
Prints comprehensive summary of Topic 9 database persistence, modular hierarchy, page structure, block types, and attached visual assets.
"""
...
def audit_topic9():
    print("=" * 80)
    print("AUDIT: FORM 4 PHYSICS — TOPIC 9: PHOTOELECTRIC EFFECT")
    print("=" * 80)

    topic = Topic.objects.filter(
        subject__name="Physics",
        subject__grade__name="Form 4",
        name__icontains="Photoelectric Effect"
    ).first()

    if not topic:
        print("ERROR: Topic 9: Photoelectric Effect not found in database!")
        return
    ...
    print("=" * 80)
    print(f"TOPIC 9 AUDIT SUMMARY: {units.count()} Units | {total_blocks_all} Total Blocks | {total_assets_all} Visual Assets")
    print("=" * 80)
