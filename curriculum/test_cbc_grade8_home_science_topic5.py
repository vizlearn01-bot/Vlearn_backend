...
Verifies:
  1. Hierarchy resolution (CBC -> Grade 8 -> Home Science -> Topic 5: CSL Class Activity)
  ...
  6. Zero bracket citations ([43], [86], [44]) and zero developer meta-language leaks
...
class TestCBCGrade8HomeScienceTopic5(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Community Service Learning (CSL) Class Activity").first()
    ...
    def test_01_hierarchy_resolution(self):
        """Test that Topic 5 resolves under CBC -> Grade 8 -> Home Science."""
        ...
        self.assertIsNotNone(self.topic, "Topic 'CSL Class Activity' not found")
        self.assertEqual(self.topic.order, 5, "Topic order must be 5")
        print("\n[QA PASS] Hierarchy correctly resolved to Topic 5: CSL Class Activity (ID: 107).")
    ...
    def test_06_zero_bracket_citations_and_meta_language(self):
        ...
        print("[QA PASS] Zero bracket citations and zero developer meta-language leaks detected across all 42 blocks.")
    ...
    def test_09_total_assets_persisted(self):
        ...
        self.assertEqual(assets.count(), 10, f"Expected 10 persisted LessonAsset records, found {assets.count()}")
        ...
        print("[QA PASS] Exactly 10 LessonAsset records verified in database (3 photos + 7 diagrams).")
