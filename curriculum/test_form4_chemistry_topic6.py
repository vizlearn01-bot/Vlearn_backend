"""
Test suite for Form 4 Chemistry Topic 6: Organic Chemistry II
"""
...
def test_form4_chemistry_topic6():
    grade = Grade.objects.get(name="Form 4")
    subject = Subject.objects.get(grade=grade, name="Chemistry")
    topic = Topic.objects.get(subject=subject, id=17)

    assert "Organic Chemistry II" in topic.name

    lessons = list(Lesson.objects.filter(topic=topic, status='published').order_by('learning_unit__order'))
    assert len(lessons) == 24, f"Expected 24 published lessons, found {len(lessons)}"
    ...
    # Verify videos in Lesson 119 and 136
    l119 = Lesson.objects.get(id=119)
    l136 = Lesson.objects.get(id=136)
    assert LessonAsset.objects.filter(lesson=l119, asset_type='youtube').exists(), "Lesson 119 missing video"
    assert LessonAsset.objects.filter(lesson=l136, asset_type='youtube').exists(), "Lesson 136 missing video"

    print("✅ All 24 Form 4 Chemistry Topic 6 lessons passed validation tests!")

if __name__ == "__main__":
    test_form4_chemistry_topic6()
