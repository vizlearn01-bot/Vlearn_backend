"""
Automated Quality Assurance Test Suite for CBC Grade 8 Agriculture — Topic 2: Water Harvesting and Storage
(Deep Pedagogical & Multi-Video Edition)

Curriculum: CBC -> Grade 8 -> Agriculture -> Topic 2: Water Harvesting and Storage

Test Coverage:
  1. Hierarchy and Topic Configuration Integrity (Grade 8, Topic Order: 2)
  2. Exactly 4 LearningUnits and 4 Published Lessons
  ...
  6. Multi-Video Integration (Videos across Lessons 1, 2, and 4 with attached LessonAssets)
  7. Zero Citation Bracket Leaks, Zero Developer Meta-Tags, Zero Raw LaTeX
  8. Formative (4) and Summative (10) MCQ Validation (14 Total Knowledge Checks)

Run:
  ./venv/bin/python curriculum/test_cbc_grade8_agriculture_topic2.py
"""
...
class TestCBCGrade8AgricultureTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic_name = "Water Harvesting and Storage"
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(units.count(), 4, f"Expected exactly 4 LearningUnits, found {units.count()}")
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all 14 interactive knowledge_check blocks across the 4 lessons."""
        mcq_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        self.assertEqual(mcq_blocks.count(), 14, f"Expected 14 MCQs (4 formative + 10 summative), found {mcq_blocks.count()}")
