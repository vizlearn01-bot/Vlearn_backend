"""
VLearn Form 4 Business Studies — Topic 9 Ingestion Engine (Determining the Net Worth of a Business)
...
from curriculum.topic9_data import TOPIC9_UNITS
...
def ingest_topic9(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 9 INGESTION ENGINE (NET WORTH & BALANCE SHEET)")
    print("=" * 80)
    ...
    # 2. Topic 9: Determining the Net Worth of a Business
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=9,
        defaults={
            "name": "Determining the Net Worth of a Business",
            "description": "Business transactions and bookkeeping definitions, debtors and creditors, classification of assets (fixed vs current) and liabilities (long-term vs current), capital (owner's equity), Bookkeeping Equation (A = C + L) transpositions, Balance Sheet structure and liquidity ordering, Solvency vs Insolvency, and Net Worth determination."
        }
    )
    if topic.name != "Determining the Net Worth of a Business":
        topic.name = "Determining the Net Worth of a Business"
        topic.description = "Business transactions and bookkeeping definitions, debtors and creditors, classification of assets (fixed vs current) and liabilities (long-term vs current), capital (owner's equity), Bookkeeping Equation (A = C + L) transpositions, Balance Sheet structure and liquidity ordering, Solvency vs Insolvency, and Net Worth determination."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 9...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC9_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 9 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L9U{unit_idx}P{page_num}B{block_num}",
                    ...
                    "metadata": {
                        "topic": "Determining the Net Worth of a Business",
                        "unit_order": unit_idx,
                        "page_number": page_num
                    }
                }
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 9 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 9")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 9 content")
    args = parser.parse_args()

    ingest_topic9(replace=args.replace)
