"""
VLearn CBC Grade 10 History — Topic 15: Peacebuilding in Kenya
...
Curriculum: CBC
Topic Order: 15
Topic Name: "Topic 3.2D: Peacebuilding in Kenya"
"""
...
from curriculum.cbc_grade10_history_topic15_data import TOPIC_15_LESSONS
...
def ingest_grade10_history_topic15(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 15: Peacebuilding in Kenya (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.2D: Peacebuilding in Kenya"
    topic_desc = "National peacebuilding activities, positive vs negative peace, NCIC mandate, constitutional pillars of conflict prevention, devolution, and the Amani River mediation simulation."

    topic = Topic.objects.filter(subject=subject, order=15).first()
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 15: {topic.name} (ID: {topic.id})")

    if replace:
        print("[*] Replacing existing Topic 15 units, lessons, blocks, and assets...")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t15_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_history_topic15(replace=True)
