...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 11: Beekeeping)
  ...
  4. Mandatory First-Card Visual Hooks (8 Tested Wikimedia URLs & attached LessonAssets)
  5. 8 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. 1 Verified Educational YouTube Video Asset (Lesson 5)
  7. Content Sanitization (0 Bracket Citations, 0 Developer Meta-Terms, 0 Raw LaTeX Leaks)
  8. Formative & Summative Scenario-Based MCQs Validation (16 MCQs with 4 Options, Valid Answer Key & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic11.py
"""
...
class TestCBCGrade10AgricultureTopic11(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Beekeeping").first()
        assert cls.topic, "Topic 11 'Beekeeping' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Beekeeping")
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video asset in Lesson 5."""
        lesson = next((l for l in self.lessons if l.learning_unit.order == 5), None)
        video_block = LessonBlock.objects.filter(
            lesson=lesson,
            block_type="suggested_video"
        ).first()
        self.assertIsNotNone(video_block, "Lesson 5 missing suggested_video block!")

        content = video_block.content or {}
        v_url = content.get("url")
        self.assertTrue(v_url and "youtube.com" in v_url, f"Lesson 5 video URL invalid: {v_url}")

        asset = video_block.assets.filter(asset_type="youtube").first()
        self.assertIsNotNone(asset, "Lesson 5 video missing attached LessonAsset!")
        self.assertEqual(asset.status, "attached")
    ...
    def test_08_formative_and_summative_mcqs(self):
        """Verify formative MCQs in Lessons 1-7 and Summative MCQs in Lesson 8 have 4 options, key, and explanation."""
        total_mcqs = 0
        for lesson in self.lessons:
        ...
        # Total MCQs = 7 (one per lesson 1-7) + 8 (summative in lesson 8) = 15 MCQs
        self.assertGreaterEqual(total_mcqs, 15, f"Expected at least 15 MCQs, found {total_mcqs}")
