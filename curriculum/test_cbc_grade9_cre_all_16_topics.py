"""
Complete Master Test Suite for CBC Grade 9 CRE — All 16 Topics (108 Lessons Total)
"""
...
class TestCBCGrade9CRECompleteCurriculum:

    def test_01_all_16_topics_and_units_count(self):
        g9 = Grade.objects.get(id=18)
        cre = Subject.objects.get(id=50, grade=g9)
        assert cre.topics.count() == 16

        expected_units = {
            1: 6, 2: 6, 3: 6, 4: 8, 5: 6, 6: 6,
            7: 6, 8: 6, 9: 8, 10: 8, 11: 8, 12: 8,
            13: 6, 14: 6, 15: 6, 16: 8
        }
    ...
    def test_02_all_108_lessons_published(self):
        ...
        for topic in cre.topics.all().order_by('order'):
            for unit in topic.learning_units.all().order_by('order'):
                ...
                total_published += 1
        assert total_published == 108

    def test_03_six_cards_per_lesson_all_108(self):
        cre = Subject.objects.get(id=50)
        for topic in cre.topics.all().order_by('order'):
            for unit in topic.learning_units.all().order_by('order'):
    ...
    def test_04_thirteen_blocks_and_correct_types_all_108(self):
        cre = Subject.objects.get(id=50)
        expected_types = {
    ...
    def test_05_three_assets_per_lesson_all_108(self):
        cre = Subject.objects.get(id=50)
        for topic in cre.topics.all().order_by('order'):
            for unit in topic.learning_units.all().order_by('order'):
                lesson = unit.lessons.get(status='published')
    ...
    def test_06_svg_diagram_xml_validity_all_108(self):
        cre = Subject.objects.get(id=50)
        for topic in cre.topics.all().order_by('order'):
            for unit in topic.learning_units.all().order_by('order'):
                lesson = unit.lessons.get(status='published')
    ...
    def test_07_mcq_structure_validity_all_108(self):
        cre = Subject.objects.get(id=50)
        for topic in cre.topics.all().order_by('order'):
            for unit in topic.learning_units.all().order_by('order'):
                lesson = unit.lessons.get(status='published')
