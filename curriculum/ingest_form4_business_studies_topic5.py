"""
VLearn Form 4 Business Studies — Topic 5 Ingestion Engine (Inflation)
...
from curriculum.topic5_data import TOPIC5_UNITS
...
@transaction.atomic
def ingest_topic5(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 5 INGESTION ENGINE (INFLATION)")
    print("=" * 80)
    ...
    topic, _ = Topic.objects.get_or_create(
        ...
        defaults={
            "name": "Inflation",
            "description": "Inflation and deflation, Consumer Price Index (C.P.I.) construction and purchasing power math, levels of inflation, demand-pull and cost-push factors, stakeholder distribution, and monetary, fiscal, and direct cost control policies."
        }
    )
    if topic.name != "Inflation":
        topic.name = "Inflation"
        topic.description = "Inflation and deflation, Consumer Price Index (C.P.I.) construction and purchasing power math, levels of inflation, demand-pull and cost-push factors, stakeholder distribution, and monetary, fiscal, and direct cost control policies."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 5...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC5_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 5 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L5U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 5 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 5")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 5 content")
    args = parser.parse_args()

    ingest_topic5(replace=args.replace)
