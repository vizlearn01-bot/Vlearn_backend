...
Decomposed into 6 Learning Units & 6 Published 5-Page Lessons:
  1. Lesson 6.1.1: Meaning, Importance, and Categories of DTP Tools
  2. Lesson 6.1.2: Interface, Navigation, and Basic File Operations
  3. Lesson 6.1.3: Inserting Shapes, Pictures, and WordArt
  4. Lesson 6.1.4: Layout, Typography, and Design Principles
  5. Lesson 6.1.5: Document Setup, Master Pages, and Template Design
  6. Lesson 6.1.6: Finalizing, Printing, and Exporting Publications
"""
...
from curriculum.cbc_grade10_ict_topic6_data import TOPIC_6_LESSONS

def clean_text(text: str) -> str:
    """Removes bracket citations, developer tags, and normalizes markdown typography."""
    if not text:
        return ""
    # Strip bracket citations e.g. [36], [258, 262]
    text = re.sub(r'\[(?:\d+|image_\d+|S\d+.*?|[\d,\s]{2,})\]', '', text)
    # Strip developer tags
    text = re.sub(r'\[(?:VISUAL:[^\]]*|SCENARIO|MCQ|MISCONCEPTION|PRACTICAL TASK)\]', '', text)
...
def ingest_grade10_ict_topic6(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 6: Desktop Publishing (Grade 10 ICT)")
    print("=" * 80)
    ...
    topic, t_created = Topic.objects.get_or_create(
        ...
        defaults={
            ...
            "description": "Desktop publishing fundamentals, software categories, workspace anatomy, graphic manipulation, C.R.A.P. design principles, typography, master pages, bleed geometry, preflight quality control, mail merge, and multi-channel publication output."
        }
    )
    if not t_created:
        topic.name = "Desktop Publishing"
        topic.description = "Desktop publishing fundamentals, software categories, workspace anatomy, graphic manipulation, C.R.A.P. design principles, typography, master pages, bleed geometry, preflight quality control, mail merge, and multi-channel publication output."
        topic.save()
    print(f"[*] Resolved Topic 6: {topic.name} (ID: {topic.id})")

    # 2. Clear Existing Data for Topic 6 if Replace is True
    if replace:
    ...
    with transaction.atomic():
        for item in TOPIC_6_LESSONS:
            u_order = item["unit_order"]
            u_name = clean_text(item["unit_name"])
            u_desc = clean_text(item["unit_description"])
            l_title = clean_text(item["lesson_title"])
            pages = item["pages"]
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_ict_t6_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print(f"TOPIC 6 (DESKTOP PUBLISHING) INGESTION COMPLETE:")
...
if __name__ == "__main__":
    ingest_grade10_ict_topic6(replace=True)
