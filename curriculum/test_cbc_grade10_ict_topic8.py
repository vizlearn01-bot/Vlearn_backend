...
class TestCBCGrade10ICTTopic8(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 8 'Digital Communication Platforms' not found under ICT!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Digital Communication Platforms")
        self.assertTrue(self.topic.description, "Topic 8 missing description")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 7, f"Expected 7 units, found {len(self.units)}")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 7 lessons have Page 1 photographic visual hooks with Wikimedia URLs and learning goals."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 7 lessons contain sanitized, responsive vector SVGs attached as LessonAssets on Page 3."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
    ...
    def test_06_youtube_video_assets(self):
        ...
        print(f"[*] Topic 8 Video Coverage: {video_count}/{len(self.lessons)} ({coverage:.1f}%)")
    ...
    def test_08_formative_mcqs(self):
        ...
        self.assertGreaterEqual(mcq_count, 14, f"Expected >= 14 MCQs across Topic 8, found {mcq_count}")

    def test_09_pedagogical_page_completeness(self):
        """Verify the full 5-page pedagogical card architecture across all 7 lessons."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
