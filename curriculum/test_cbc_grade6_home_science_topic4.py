...
CBC Grade 6 Home Science — Topic 4: Clothing and Laundry

Validates:
1. Hierarchy Resolution (CBC -> Grade 6 -> Home Science -> Topic 4: Clothing and Laundry, Order: 4)
2. Learning Units & Lessons Count (Exactly 6 Units, 6 Published Lessons)
3. Page Count (Exactly 8 pages per lesson = 48 total pages)
4. Card 1 Photographic Visual Hooks Live HTTP 200 Verification (>10KB)
5. Custom Vector SVGs Validity (Valid XML, sanitized, viewBox="0 0 800 450", 12 total SVGs)
...
9. Attached LessonAssets Count (6 photos + 12 diagrams = 18 assets)
"""
...
class TestCBCGrade6HomeScienceTopic4(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Clothing and Laundry").first()
    ...
    def test_01_hierarchy_resolution(self):
        ...
        self.assertIsNotNone(self.topic, "Topic 4 (Clothing and Laundry) missing!")
        self.assertEqual(self.topic.order, 4, f"Expected Topic 4 order to be 4, got {self.topic.order}")
        print(f"\n[QA PASS] Hierarchy correctly resolved to Grade 6 Topic 4: {self.topic.name} (ID: {self.topic.id}).")

    def test_02_units_and_lessons_count(self):
        units = self.topic.learning_units.all().order_by("order")
        self.assertEqual(units.count(), 6, f"Expected 6 Learning Units, got {units.count()}")
        lessons = Lesson.objects.filter(topic=self.topic, status="published")
        self.assertEqual(lessons.count(), 6, f"Expected 6 published Lessons, got {lessons.count()}")
        print(f"[QA PASS] Exactly 6 Learning Units and 6 Published Lessons verified.")
    ...
    def test_03_page_count_per_lesson(self):
        ...
        self.assertEqual(total_pages, 48, f"Expected 48 total pages, got {total_pages}")
        print(f"[QA PASS] Exactly 48 total pages verified (8 pages per lesson).")
    ...
    def test_04_card_1_photographic_hooks(self):
        ...
        print(f"[QA PASS] All 6 Card 1 photographic hooks verified live with HTTP 200 responses.")
    ...
    def test_05_custom_vector_svgs_validity(self):
        ...
        self.assertEqual(svg_blocks.count(), 12, f"Expected 12 diagram blocks, found {svg_blocks.count()}")
        ...
        print(f"[QA PASS] All 12 Vector SVGs verified valid, responsive (800x450), and sanitized.")
    ...
    def test_08_scenario_knowledge_checks(self):
        ...
        self.assertEqual(kc_blocks.count(), 6)
        for kb in kc_blocks:
            c = kb.content or {}
        ...
        print(f"[QA PASS] All 6 scenario knowledge checks validated with 4 options, valid index, and detailed explanation.")

    def test_09_attached_assets_count(self):
        total_assets = LessonAsset.objects.filter(lesson__topic=self.topic).count()
        # 6 photos + 12 diagrams = 18 assets
        self.assertEqual(total_assets, 18, f"Expected 18 LessonAsset records, found {total_assets}")
        print(f"[QA PASS] Exactly 18 LessonAsset records verified in database (6 photos + 12 diagrams).")
