...
Tests:
  1. Hierarchy & Database Integrity (CBC -> Grade 10 -> Geography -> Topic 1: Introduction to Geography)
  2. 8 Learning Units & 8 Published Lessons Validation
  3. Dynamic Card & Block Structure Integrity (all lessons >= 3 pages, non-empty content)
  4. Mandatory First-Card Visual Hooks (8 Tested HTTP 200 URLs & attached LessonAssets)
  5. 8 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. Verified Educational YouTube Video Asset
  7. Content Sanitization (0 Bracket Citations, 0 Developer Meta-Terms, 0 Raw LaTeX Leaks)
  8. Formative & Summative Scenario-Based MCQs Validation (Valid Answer Keys & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_geography_topic1.py
"""
...
class TestCBCGrade10GeographyTopic1(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 1 'Introduction to Geography' not found under Geography!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Introduction to Geography")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 8 lessons have Page 1 photographic visual hooks with HTTP 200 URLs & attached LessonAssets."""
    ...
    def test_05_custom_vector_svgs(self):
        """Verify all 8 lessons have valid custom vector SVGs with viewBox and attached LessonAssets."""
        for lesson in self.lessons:
            u_order = lesson.learning_unit.order
            ...
            self.assertTrue(svg_text, f"Lesson {u_order} diagram block missing svg_content!")
            self.assertIn("<svg", svg_text)
            self.assertIn("viewBox=", svg_text)
            self.assertNotIn("<?xml", svg_text)

            svg_asset = LessonAsset.objects.filter(lesson=lesson, asset_type="diagram", blocks=diag_block).first()
            self.assertIsNotNone(svg_asset, f"Lesson {u_order} missing attached LessonAsset for diagram!")
            self.assertEqual(svg_asset.status, "attached")

    def test_06_video_assets(self):
        """Verify Lesson 1 has attached educational YouTube video asset."""
        l1 = self.lessons[0]
        v_block = LessonBlock.objects.filter(lesson=l1, block_type="suggested_video").first()
        self.assertIsNotNone(v_block, "Lesson 1 missing suggested_video block!")

        v_asset = LessonAsset.objects.filter(lesson=l1, asset_type="youtube", blocks=v_block).first()
        self.assertIsNotNone(v_asset, "Lesson 1 missing attached YouTube LessonAsset!")
        self.assertIn("youtube.com", v_asset.url)

    def test_07_content_purification(self):
        """Scan all blocks across Topic 1 for zero prompt leaks, zero bracket citations, and clean formatting."""
        forbidden_patterns = [
            r'\[VISUAL:\s*.*?\]',
            r'\[\d+\]',
            r'\[image_\d+\]',
        ...
        for block in all_blocks:
            ...
            for pat in forbidden_patterns:
                ...
                title_match = re.search(pat, block.title or "", re.IGNORECASE)
                self.assertIsNone(title_match, f"Block {block.id} title contains forbidden tag matching '{pat}'")
    ...
    def test_08_formative_mcqs_validation(self):
        ...
        self.assertGreaterEqual(mcq_blocks.count(), 10, "Expected at least 10 MCQs across Topic 1")
