#!/usr/bin/env python3
"""
VLearn Form 4 Mathematics — Topic 5: Longitudes and Latitudes Test Suite
==========================================================================
Validates:
  ...
  5. Content quality (zero developer leakages, zero raw math arrays, key spherical geometry concepts present)
...
class TestHierarchy(unittest.TestCase):
    """Verify the database hierarchy for Topic 5."""
    ...
    def test_topic_exists(self):
        topic = Topic.objects.filter(name__icontains="Longitudes and Latitudes", subject__name="Mathematics").first()
        self.assertIsNotNone(topic, "Topic 'Longitudes and Latitudes' not found")
        self.assertEqual(topic.order, 5, "Topic order should be 5")

    def test_exactly_4_learning_units(self):
        topic = Topic.objects.filter(name__icontains="Longitudes and Latitudes").first()
...
class TestBlockCounts(unittest.TestCase):
    ...
    def setUp(self):
        self.topic = Topic.objects.filter(name__icontains="Longitudes and Latitudes").first()
    ...
    def test_minimum_total_blocks(self):
        ...
        self.assertGreaterEqual(
            total_blocks, 40,
            f"Expected at least 40 blocks across Topic 5, found {total_blocks}"
        )
...
class TestContentQuality(unittest.TestCase):
    ...
    def test_key_spherical_concepts_present(self):
        all_text = " ".join([json.dumps(b.content, default=str) for b in self.blocks])
        key_terms = [
            "great circle", "small circle", "nautical mile", "knot",
            "meridian", "equator", "parallel", "longitude", "latitude"
        ]
        for term in key_terms:
            self.assertIn(
                term.lower(), all_text.lower(),
                f"Key spherical geometry concept '{term}' not found in Topic 5 content"
            )
...
if __name__ == "__main__":
    print("=" * 80)
    print("Form 4 Mathematics Topic 5 (Longitudes and Latitudes) — Validation Test Suite")
    print("=" * 80)
