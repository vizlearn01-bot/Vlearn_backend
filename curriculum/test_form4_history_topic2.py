"""
VLearn Form 4 History — Automated Test Suite for Topic 2: International Relations

Verifies:
  1. Target Subject, Grade, Curriculum mapping (History, Form 4, 844)
  2. Topic 2 existence and ordering (Order: 2)
  3. Exact 7 Learning Units and 7 Lessons
  4. Exact 111 Pages across the 7 Lessons (14, 14, 16, 18, 15, 16, 18)
  ...
  7. Zero bracket citations ([143], [230], [1]) across all blocks
  8. Zero internal developer/generation metadata leaks
  9. 100% LessonAsset associations (Images + Videos)

Usage:
  ./venv/bin/python curriculum/test_form4_history_topic2.py
"""
...
def test_form4_history_topic2():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 HISTORY TOPIC 2 (INTERNATIONAL RELATIONS)")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="International Relations").first()
    assert topic, "Topic 'International Relations' not found!"
    ...
    print("[PASS] 4.1 Every single lesson in Topic 2 has an establishing real-world visual on Page 1.")
    ...
    expected_pages = {
        1: 14, # Meaning, Forms, and Benefits of International Relations
        2: 14, # International Organizations: IGOs and NGOs
        3: 16, # The League of Nations: Origins, Achievements, Collapse
        4: 18, # The United Nations Organization: Structure, Impact, Challenges
        5: 15, # The Commonwealth of Nations: Evolution, Impact, Dynamics
        6: 16, # The Non-Aligned Movement: Active Neutrality and Global Role
        7: 18  # The Cold War: Superpower Rivalry, Proxy Theatres, African Impact
    }
    ...
    assert total_pages == 111, f"Total pages is {total_pages}, expected 111"
    print(f"[PASS] 5. Total Pages across all 7 Lessons: {total_pages} (Matches target: 111)")
    ...
    assert len(video_blocks) >= 4, f"Expected at least 4 suggested_video blocks, found {len(video_blocks)}"
    ...
    assert len(assets) >= 12, f"Expected at least 12 LessonAsset records, found {len(assets)}"
    for a in assets:
    ...
    # 9. Check for Bracket Citation Leakage ([143], [230], [1])
    ...
    print("=" * 80)
    print(f"[ALL TESTS PASSED] Form 4 History Topic 2 is fully verified, robust, and production-ready!")
