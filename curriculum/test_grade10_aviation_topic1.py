"""
VLearn Automated QA Suite: Grade 10 Aviation — Topic 1
...
class TestGrade10AviationTopic1(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.subject = Subject.objects.get(id=44)
        cls.topic = Topic.objects.get(id=247, subject=cls.subject)
    ...
    def test_topic_hierarchy(self):
        """Verify Topic 1 exists with correct attributes."""
        self.assertEqual(self.topic.name, "Foundations of Aviation Technology")
        self.assertEqual(self.topic.order, 0)
        self.assertEqual(self.units.count(), 5, "Expected 5 LearningUnits under Topic 1")
        self.assertEqual(self.lessons.count(), 5, "Expected 5 Lessons under Topic 1")
    ...
    def test_card_atomicity_and_blocks(self):
        ...
        for lesson in self.lessons:
            ...
            self.assertIn("<svg", diag_block.content.get("svg", ""), f"Lesson {lesson.id} SVG XML missing")
