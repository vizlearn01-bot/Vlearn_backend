...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 13: Tools and Equipment)
  2. 14 Learning Units & 14 Published Lessons Validation
  3. Dynamic Card & Block Structure Integrity (all lessons >= 4 pages, non-empty content)
  4. Mandatory First-Card Visual Hooks (14 Tested Wikimedia URLs & attached LessonAssets)
  5. 8 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. 1 Verified Educational YouTube Video Asset (Lesson 11)
  7. Content Sanitization (0 Bracket Citations, 0 Developer Meta-Terms, 0 Raw LaTeX Leaks)
  8. Formative & Summative Scenario-Based MCQs Validation (21 MCQs with 4 Options, Valid Answer Key & Explanations)

Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic13.py
"""
...
class TestCBCGrade10AgricultureTopic13(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Tools and Equipment").first()
        assert cls.topic, "Topic 13 'Tools and Equipment' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.order, 13)
        self.assertEqual(self.topic.name, "Tools and Equipment")

    def test_02_units_and_published_lessons_count(self):
        """Verify exactly 14 LearningUnits and 14 published Lessons exist in sequential order."""
        self.assertEqual(len(self.units), 14, f"Expected 14 units, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 14, f"Expected 14 lessons, found {len(self.lessons)}")
    ...
    def test_04_card_1_visual_hooks(self):
        """Verify all 14 lessons have Page 1 photographic visual hooks with HTTP 200 URLs & attached LessonAssets."""
        for lesson in self.lessons:
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 3, 6, 9, 10, 11, 13, 14]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video asset in Lesson 11."""
        lesson = next((l for l in self.lessons if l.learning_unit.order == 11), None)
        video_block = LessonBlock.objects.filter(
            lesson=lesson,
            block_type="suggested_video"
        ).first()
        self.assertIsNotNone(video_block, "Lesson 11 missing suggested_video block!")

        content = video_block.content or {}
        v_url = content.get("url")
        self.assertTrue(v_url and "youtube.com" in v_url, f"Lesson 11 video URL invalid: {v_url}")

        asset = video_block.assets.filter(asset_type="youtube").first()
        self.assertIsNotNone(asset, "Lesson 11 video missing attached LessonAsset!")
        self.assertEqual(asset.status, "attached")
    ...
    def test_08_formative_and_summative_mcqs(self):
        """Verify formative MCQs in Lessons 1-13 and Summative MCQs in Lesson 14 have 4 options, key, and explanation."""
        total_mcqs = 0
        for lesson in self.lessons:
        ...
        # Total MCQs = 13 (one per lesson 1-13) + 8 (summative in lesson 14) = 21 MCQs
        self.assertGreaterEqual(total_mcqs, 21, f"Expected at least 21 MCQs, found {total_mcqs}")
