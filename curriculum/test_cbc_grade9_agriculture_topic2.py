"""
Automated Quality Assurance Test Suite for CBC Grade 9 Agriculture — Topic 2: Conserving Leftover Food
Curriculum: CBC -> Grade 9 -> Agriculture -> Topic 2: Conserving Leftover Food

Test Coverage:
  1. Hierarchy and Topic Configuration Integrity
  2. Exactly 9 LearningUnits and 9 Published Lessons
  ...
  6. Topic Video Review (YouTube ID: 0EErECU8PXU, verified status, attached LessonAsset)
  7. Zero Citation Bracket Leaks, Zero Developer Meta-Tags, Zero Raw LaTeX
  8. Formative (9) and Summative (10) MCQ Validation (19 Total Knowledge Checks)

Run:
  ./venv/bin/python curriculum/test_cbc_grade9_agriculture_topic2.py
"""
...
class TestCBCGrade9AgricultureTopic2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic_name = "Conserving Leftover Food (Meaning, Importance, and Household Methods)"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.order, 2, "Topic 2 order must be 2")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(units.count(), 9, f"Expected exactly 9 LearningUnits, found {units.count()}")
    ...
    def test_08_formative_and_summative_mcqs_validation(self):
        """Validate all 19 interactive knowledge_check blocks across the 9 lessons."""
        mcq_blocks = LessonBlock.objects.filter(lesson__topic=self.topic, block_type="knowledge_check")
        self.assertEqual(mcq_blocks.count(), 19, f"Expected 19 MCQs (9 formative + 10 summative), found {mcq_blocks.count()}")
