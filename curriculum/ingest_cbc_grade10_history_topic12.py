...
Curriculum: CBC
Topic Order: 12
Topic Name: "Topic 3.2C: The First Industrial Revolution and Africa"
"""
...
from curriculum.cbc_grade10_history_topic12_data import TOPIC_12_LESSONS
...
def ingest_grade10_history_topic12(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 12: The First Industrial Revolution and Africa (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.2C: The First Industrial Revolution and Africa"
    topic_desc = "Enabling factors in Britain and the USA, industrial impacts on Africa, Walter Rodney's underdevelopment thesis, industrial weapons of conquest, and the Scramble for Africa."

    topic = Topic.objects.filter(subject=subject, order=12).first()
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 12: {topic.name} (ID: {topic.id})")

    if replace:
        print("[*] Replacing existing Topic 12 units, lessons, blocks, and assets...")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t12_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
...
if __name__ == "__main__":
    ingest_grade10_history_topic12(replace=True)
