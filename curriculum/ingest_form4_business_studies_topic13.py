"""
VLearn Form 4 Business Studies — Topic 13 Ingestion Engine (Cash Book)
...
from curriculum.topic13_data import TOPIC13_UNITS
...
@transaction.atomic
def ingest_topic13(replace=False):
    print("=" * 80)
    print("STARTING FORM 4 BUSINESS STUDIES TOPIC 13 INGESTION ENGINE (CASH BOOK)")
    print("=" * 80)
    ...
    # 2. Topic 13: Cash Book
    topic, _ = Topic.objects.get_or_create(
        subject=subject,
        order=13,
        defaults={
            "name": "Cash Book",
            "description": "Dual role of the Cash Book (Book of Original Entry + Ledger Account), 5 primary types of cash books, Single-Column Cash Book structures & balancing, Two-Column Cash Books (side-by-side cash & bank tracking with independent column balancing), Three-Column Cash Books (Discount Allowed on Debit side vs Discount Received on Credit side, and the Golden Rule: Discounts are totalled, never balanced!), Contra entries (marked with 'C' in folio for internal cash transfers), dishonoured cheques (bounced cheque causes & credit bank reversal entries), and KCSE essay mastery."
        }
    )
    if topic.name != "Cash Book":
        topic.name = "Cash Book"
        topic.description = "Dual role of the Cash Book (Book of Original Entry + Ledger Account), 5 primary types of cash books, Single-Column Cash Book structures & balancing, Two-Column Cash Books (side-by-side cash & bank tracking with independent column balancing), Three-Column Cash Books (Discount Allowed on Debit side vs Discount Received on Credit side, and the Golden Rule: Discounts are totalled, never balanced!), Contra entries (marked with 'C' in folio for internal cash transfers), dishonoured cheques (bounced cheque causes & credit bank reversal entries), and KCSE essay mastery."
        topic.save()

    print(f"[*] Topic: {topic.name} (ID: {topic.id}, Order: {topic.order})")

    if replace:
        print("[!] --replace flag supplied: Cleaning existing units and lessons for Topic 13...")
    ...
    for unit_idx, unit_data in enumerate(TOPIC13_UNITS, start=1):
        ...
        learning_unit = LearningUnit.objects.create(
            topic=topic,
            name=unit_name,
            description=f"Form 4 Business Studies Topic 13 Unit {unit_idx}: {unit_name}",
            order=unit_idx
        )
        ...
        for page in unit_data["pages"]:
            ...
            for block_num, b_data in enumerate(page["blocks"], start=1):
                ...
                block_kwargs = {
                    "lesson": lesson,
                    "block_id": f"L13U{unit_idx}P{page_num}B{block_num}",
    ...
    print("=" * 80)
    print("[SUCCESS] Form 4 Business Studies Topic 13 Ingestion Complete!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Form 4 Business Studies Topic 13")
    parser.add_argument("--replace", action="store_true", help="Replace existing Topic 13 content")
    args = parser.parse_args()

    ingest_topic13(replace=args.replace)
