...
Grade: Grade 10
Curriculum: CBC
Topic 6: Topic 1.6: National Integration
"""
...
from curriculum.cbc_grade10_history_topic6_data import TOPIC_6_LESSONS
...
def ingest_grade10_history_topic6(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 6: National Integration (Grade 10 History)")
    print("=" * 80)
    ...
    if not grade:
        grade, _ = Grade.objects.get_or_create(
            ...
            name="Grade 10",
            defaults={"level": 10, "description": "Senior School Grade 10"}
        )
    ...
    if not subject:
        subject, _ = Subject.objects.get_or_create(
            grade=grade,
            name="History",
            defaults={"description": "Senior Secondary History Curriculum (Grade 10 CBC)"}
        )
    print(f"[*] Resolved Subject: {subject.name} (ID: {subject.id}) in Grade: {grade.name}")

    topic_name = "Topic 1.6: National Integration"
    topic_desc = (
        "Meaning and dimensions of national integration, historical and contemporary barriers including "
        "colonial divide-and-rule, negative ethnicity, and regional disparities, constitutional and institutional "
        "strategies (Article 10, Devolution, NCIC, Equalisation Fund), and applied citizenship inquiry projects."
    )
    ...
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 6: {topic.name} (ID: {topic.id})")
        ...
        for item in TOPIC_6_LESSONS:
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    elif b_type == "misconception_card":
                        b_type = "common_misconception"
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t6_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 6 INGESTION COMPLETE:")
...
if __name__ == "__main__":
    ingest_grade10_history_topic6(replace=True)
