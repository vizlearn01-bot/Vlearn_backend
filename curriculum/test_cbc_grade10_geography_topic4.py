...
Tests:
  1. Hierarchy & Database Integrity (CBC -> Grade 10 -> Geography -> Topic 4: Geographic Information System (GIS))
  ...
  5. 13 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. Verified Educational YouTube Video Asset (Lesson 4)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_geography_topic4.py
"""
...
class TestCBCGrade10GeographyTopic4(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        assert cls.topic, "Topic 4 'Geographic Information System (GIS)' not found under Geography!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Geographic Information System (GIS)")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        self.assertEqual(diag_blocks.count(), 13, f"Expected 13 diagram blocks in Topic 4, found {diag_blocks.count()}")
    ...
    def test_06_video_assets(self):
        """Verify Lesson 4 has attached educational YouTube video asset."""
        l4 = next(l for l in self.lessons if l.learning_unit.order == 4)
        v_block = LessonBlock.objects.filter(lesson=l4, block_type="video").first()
        self.assertIsNotNone(v_block, "Lesson 4 missing video block!")

        v_asset = LessonAsset.objects.filter(lesson=l4, asset_type="youtube", blocks=v_block).first()
        self.assertIsNotNone(v_asset, "Lesson 4 missing attached YouTube LessonAsset!")
        self.assertIn("youtube.com", v_asset.url)

    def test_07_content_purification(self):
        """Scan all blocks across Topic 4 for zero prompt leaks, zero bracket citations, and clean formatting."""
        forbidden_patterns = [
            r'\[VISUAL:\s*.*?\]',
            r'\[\d+\]',
            r'\[image_\d+\]',
    ...
    def test_08_formative_mcqs_validation(self):
        ...
        self.assertGreaterEqual(mcq_blocks.count(), 26, f"Expected at least 26 MCQs across Topic 4, got {mcq_blocks.count()}")
