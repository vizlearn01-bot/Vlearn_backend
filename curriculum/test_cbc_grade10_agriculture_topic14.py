...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 14: Product Processing and Value Addition)
  ...
  4. Mandatory First-Card Visual Hooks (12 Tested Wikimedia URLs & attached LessonAssets)
  5. 8 Custom Sanitized Responsive Vector SVGs (viewBox, sanitization & attached LessonAssets)
  6. 1 Verified Educational YouTube Video Asset (Lesson 9)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic14.py
"""
...
class TestCBCGrade10AgricultureTopic14(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Product Processing and Value Addition").first()
        assert cls.topic, "Topic 14 'Product Processing and Value Addition' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Product Processing and Value Addition")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 4, 5, 6, 8, 9, 12]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video asset in Lesson 9."""
        lesson = next((l for l in self.lessons if l.learning_unit.order == 9), None)
        video_block = LessonBlock.objects.filter(
            lesson=lesson,
            block_type="suggested_video"
        ).first()
        self.assertIsNotNone(video_block, "Lesson 9 missing suggested_video block!")

        content = video_block.content or {}
        v_url = content.get("url")
        self.assertTrue(v_url and "youtube.com" in v_url, f"Lesson 9 video URL invalid: {v_url}")

        asset = video_block.assets.filter(asset_type="youtube").first()
        self.assertIsNotNone(asset, "Lesson 9 video missing attached LessonAsset!")
        self.assertEqual(asset.status, "attached")
