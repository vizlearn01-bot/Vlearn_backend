...
from curriculum.cbc_grade10_history_topic10_data import TOPIC_10_LESSONS
...
def ingest_grade10_history_topic10(replace=True):
    print("=" * 80)
    print("INGESTING TOPIC 10: Great Revolutions — The French Revolution (Grade 10 History)")
    print("=" * 80)
    ...
    topic_name = "Topic 3.1: Great Revolutions — The French Revolution"
    topic_desc = "Social hierarchies of the Ancien Régime, causes, key turning points, domestic transformations, global ripples, and modern lessons for constitutional citizenship."
    ...
    if not topic:
        ...
        topic.save()
        print(f"[*] Resolved & Updated Topic 10: {topic.name} (ID: {topic.id})")
            ...
            for page_idx, page_blocks in enumerate(pages, start=1):
                ...
                for comp_idx, block_def in enumerate(page_blocks, start=1):
                    ...
                    block = LessonBlock.objects.create(
                        lesson=lesson,
                        block_id=f"g10_hist_t10_u{u_order}_p{page_idx}_b{comp_idx}",
                        block_type=b_type,
    ...
    print("=" * 80)
    print("TOPIC 10 INGESTION COMPLETE:")
