...
Tests:
  1. Hierarchy & Database Integrity (Curriculum -> Grade 10 -> Agriculture -> Topic 4: Field Management Practices)
  ...
  6. 2 Verified Educational YouTube Video Assets (Lesson 2 & Lesson 7)
...
Usage:
  ./venv/bin/python curriculum/test_cbc_grade10_agriculture_topic4.py
"""
...
class TestCBCGrade10AgricultureTopic4(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, name="Field Management Practices").first()
        assert cls.topic, "Topic 4 'Field Management Practices' not found under Agriculture!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Field Management Practices")
    ...
    def test_05_custom_vector_svgs(self):
        ...
        expected_svg_units = [1, 2, 3, 4, 5, 7, 8, 9]
    ...
    def test_06_video_review_assets(self):
        """Verify educational YouTube video assets in Lessons 2 and 7."""
        for u_order in [2, 7]:
            lesson = next((l for l in self.lessons if l.learning_unit.order == u_order), None)
