"""
VLearn Form 4 Geography — Automated Test Suite for Topic 3: Wildlife and Tourism

Verifies:
  1. Hierarchy mapping: 844 -> Form 4 -> Geography (ID: 18)
  2. Topic 3 existence and ordering (Topic Order: 3)
  3. Exact 6 Learning Units and 6 published Lessons
  4. Exact 80 Total Pages across all 6 Lessons:
     - Lesson 1: 12 Pages
     - Lesson 2: 12 Pages
     - Lesson 3: 14 Pages
     - Lesson 4: 12 Pages
     - Lesson 5: 14 Pages
     - Lesson 6: 16 Pages
  5. 100% presence of sanitized SVGs on all 16 diagram blocks
  6. 100% presence of verified Wikimedia URLs and full attributions on all 6 photo blocks
  7. Zero bracket citation leaks ([13], [25], [119]) across all blocks
  8. Zero developer meta-language leaks in titles or text

Usage:
  ./venv/bin/python curriculum/test_form4_geography_topic3.py
"""
...
def test_form4_geography_topic3():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 GEOGRAPHY TOPIC 3 (WILDLIFE AND TOURISM)")
    print("=" * 80)
    ...
    # 2. Check Topic
    topic = Topic.objects.filter(subject=subject, name="Wildlife and Tourism").first()
    assert topic, "Topic 'Wildlife and Tourism' not found!"
    ...
    expected_pages = {
        1: 12, # Foundations & Factors
        2: 12, # Protected Area Systems
        3: 14, # Significance, Threats & Management
        4: 12, # Tourism Concepts & Factors
        5: 14, # Coastal & Inland Attractions
        6: 16  # Comparative Analysis & Expansion
    }
    ...
    assert total_pages == 80, f"Total pages is {total_pages}, expected 80"
    print(f"[PASS] 5. Total Pages across all 6 Lessons: {total_pages} (Matches target: 80)")

    # 6. Check Visual Diagram SVGs
    diagram_blocks = [b for b in all_blocks if b.block_type == "suggested_diagram"]
    assert len(diagram_blocks) >= 16, f"Expected at least 16 diagram blocks, found {len(diagram_blocks)}"
    ...
    # 8. Check for Bracket Citation Leakage ([13], [25], [119])
    ...
    print("=" * 80)
    print(f"[ALL TESTS PASSED] Form 4 Geography Topic 3 (Wildlife and Tourism) is fully verified and production-ready!")
