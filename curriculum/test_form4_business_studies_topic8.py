"""
VLearn Form 4 Business Studies — Automated Test Suite: Topic 8 (Economic Integration)
"""
...
def run_tests():
    print("=" * 80)
    print("RUNNING AUTOMATED TEST SUITE: FORM 4 BUSINESS STUDIES TOPIC 8 (ECONOMIC INTEGRATION)")
    print("=" * 80)
    ...
    assert topic is not None, "Topic 8 (Economic Integration) not found!"
    assert topic.name == "Economic Integration", f"Unexpected topic name: {topic.name}"
    ...
    assert units.count() == 3, f"Expected 3 Learning Units, found {units.count()}"
    
    print(f"[PASS] 3. Found exactly 3 Learning Units:")
    for u in units:
        print(f"       - Unit {u.order}: {u.name}")

    # 4. Lessons test
    lessons = Lesson.objects.filter(topic=topic).order_by('learning_unit__order')
    assert lessons.count() == 3, f"Expected 3 Lessons, found {lessons.count()}"
    
    print(f"[PASS] 4. Found exactly 3 Published Lessons:")
    for l in lessons:
    ...
    assert total_pages == 30, f"Total pages across topic is {total_pages}, expected 30!"
    print(f"[PASS] 5. Total Pages across all 3 Lessons: {total_pages} (Matches target: 30)")
    ...
    assert assets.count() >= 9, f"Expected at least 9 assets, found {assets.count()}"
    ...
    assert worked_examples >= 3, f"Expected at least 3 worked examples, got {worked_examples}"
    assert knowledge_checks >= 3, f"Expected at least 3 knowledge checks, got {knowledge_checks}"
    assert tables >= 2, f"Expected at least 2 comparison tables, got {tables}"

    print(f"[PASS] 11. Verified {worked_examples} worked examples, {knowledge_checks} knowledge checks, and {tables} comparison tables.")

    print("=" * 80)
    print("[ALL TESTS PASSED] Form 4 Business Studies Topic 8 is 100% verified and production-ready!")
    print("=" * 80)
