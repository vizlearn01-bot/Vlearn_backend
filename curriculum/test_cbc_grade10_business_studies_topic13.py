...
class TestCBCGrade10BusinessStudiesTopic13(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.topic = Topic.objects.filter(subject=cls.subject, order=13).first()
        assert cls.topic, "Topic 13 'Business Transactions' not found in database!"
    ...
    def test_01_hierarchy_integrity(self):
        ...
        self.assertEqual(self.topic.name, "Business Transactions")
    ...
    def test_02_units_and_published_lessons_count(self):
        ...
        self.assertEqual(len(self.units), 5, f"Expected 5 units in Topic 13, found {len(self.units)}")
        self.assertEqual(len(self.lessons), 5, f"Expected 5 lessons in Topic 13, found {len(self.lessons)}")
    ...
    def test_06_youtube_video_assets(self):
        ...
        for lesson in self.lessons:
            ...
            if yt_asset or yt_block:
                video_count += 1
        self.assertEqual(video_count, len(self.lessons), f"Expected 100% video coverage in Topic 13, found {video_count}/{len(self.lessons)}")
